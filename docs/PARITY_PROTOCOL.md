...
- **A continuation's constructor header and table intern with the constructor (fix).** kotlinc interns
  a method's header — name, descriptor, `Signature` — before it visits the body, and that method's
  `LocalVariableTable` names right after it, before the next method. krusty interned the primary
  constructor's header at `add_method_sig`, i.e. AFTER everything its body touched (on a continuation
  that put `<init>` behind the `this$0` field reference it stores), and batched every method's table
  names into a trailing debug pass.
  With this, the spill positions and the pool's visit order, a suspending function's continuation class
...
- **A `return@label` carries its lambda's expected result type (fix).** An inline lambda's TAIL
  expression is checked against the result type the call site expects; a labelled return out of the
  same lambda was checked with no expectation at all. `return@run emptyList()` therefore had no
  element type to infer from, and merging it with a concrete tail produced `Any` —
  `type mismatch: inferred type is Any but List<Long> was expected`, rejecting a declaration against
  its own declared type. Both exits of a lambda are the same result position and are now judged
  against the same expectation, at BOTH sites: the statement form and the expression form
  (`xs ?: return@let emptyList()`). The discriminator that isolated it: swapping the two exits —
  a concrete labelled return with an `emptyList()` TAIL always worked, because only the tail had the
  expectation.
...
- **An absent type-parameter bound is `Any?`, not `Any` (fix).** The member call path synthesizes a
  generic signature for the callee and filled each ABSENT type-parameter bound with a non-null `Any`.
  ...
  was fixed to `Status`, which became the lambda's expected result. Three sites filled the bound this
  way and now agree on Kotlin's implicit `Any?`. An explicit type argument bypassed inference
  entirely and the TOP-LEVEL path uses the declared bounds directly, which is why only members failed
  and why the defect survived so long. A DECLARED `R : Any` still rejects a nullable result: the
  narrowing rule is correct, only the fabricated bound was wrong.
