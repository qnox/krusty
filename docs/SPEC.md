...
- **An `actual` with no `expect` to actualize is an error, named by a declaration renderer.** With
  ...
  and a nearer member hides an inherited one only when it overrides it (the same complete input
  signature): a declared `choose(String)` leaves an inherited `choose(Int)` in the scope to
  actualize `expect fun choose(value: Int)`. A private supertype member is
  ...
  `inherited_overload_beside_a_declared_one_actualizes_its_expect`,
...
- **A `super` call to a `suspend` member on the current instance is a suspension point.** The
  ...
  rather than dispatching back to the override. A dependency publishes the member's classfile
  parameters with the trailing continuation; the super call names the logical ones, as its
  descriptor does. Declaration in the same file, a sibling file or a dependency, reached through a
  class or an interface (`super<I>.f()`), all run, and the override's instructions match kotlinc's.
