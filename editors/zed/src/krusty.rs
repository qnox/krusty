...
impl KrustyExtension {
    fn ensure_managed_binary(&mut self, id: &LanguageServerId) -> Result<String> {
        ...
        let index = pick_asset_index(&names, triple, windows).ok_or_else(|| {
            format!("release {} has no krusty-lsp asset for {triple}", release.version)
        })?;
        ...
        if !is_file(&binary_path) {
            ...
            let file_type = if windows { DownloadedFileType::Zip } else { DownloadedFileType::GzipTar };
    ...
    /// cached path if still present, otherwise the highest-sorting cached version.
    fn offline_binary(&self, windows: bool) -> Option<String> {
        ...
        dirs.into_iter().rev().find_map(|dir| {
            let candidate = format!("{dir}/{}", binary_filename(windows));
...
mod tests {
    ...
    fn target_triple_covers_every_shipped_target() {
        assert_eq!(target_triple(Os::Mac, Architecture::Aarch64).unwrap(), "aarch64-apple-darwin");
        assert_eq!(target_triple(Os::Mac, Architecture::X8664).unwrap(), "x86_64-apple-darwin");
        assert_eq!(
    ...
    fn pick_asset_selects_lsp_for_triple() {
        ...
        assert_eq!(names[index], "krusty-lsp-2.4.20-build.3-aarch64-apple-darwin.tar.gz");
    }
    ...
    fn pick_asset_never_selects_compiler_or_extension_asset() {
        ...
        let names =
            vec!["krusty-2.4.20-build.3-aarch64-apple-darwin.tar.gz".to_string(), "krusty-zed-2.4.20-build.3.tar.gz".to_string()];
    ...
    fn pick_asset_honors_windows_zip_extension() {
        ...
        assert_eq!(names[index], "krusty-lsp-2.4.20-build.3-x86_64-pc-windows-msvc.zip");
