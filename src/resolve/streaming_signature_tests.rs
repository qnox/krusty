...
#[test]
fn delegated_lazy_property_preserves_nested_builder_inference() {
    assert_jvm_streaming_frontend(
        r#"// WITH_STDLIB
           interface A {
...
#[test]
fn a_shadowing_callee_without_a_contract_does_not_narrow_in_pass_one() {
    let source = r#"// WITH_STDLIB
class Runtime(val status: String)
...
"#;
    let inputs = [SourceInput::kotlin(source).with_file_stem("ShadowedContractCallee")];
    let mut paths = crate::toolchain::classpath_jars_for(source);
    paths.push(
        crate::toolchain::jdk_modules()
            .expect("shadowed contract callee regression requires the configured JDK"),
    );
    let classpath = std::rc::Rc::new(crate::jvm::classpath::Classpath::new(paths));
    let mut diagnostics = DiagSink::new();
    ...
    // The same-module `assertNotNull` has no contract: `runtime` stays nullable and the member
    // read is rejected, as kotlinc rejects it.
    assert!(
        diagnostics
            .diags
            .iter()
            .any(|diagnostic| diagnostic.msg.contains("nullable receiver")
                || diagnostic.msg.contains("unresolved reference 'status'")),
        "{:?}",
        diagnostics.diags
    );
}
