...
//!
//! A value parameter mentions a formal in an INVARIANT position and fixes it exactly. An extension
//! receiver does not: it only requires that the receiver be assignable to the formal's
//! instantiation, which is a LOWER bound. The two therefore belong in one constraint set.
//!
...
impl Checker<'_> {
    ...
    pub(super) fn mapped_generic_call_bindings(
        ...
        // value parameter mentions a formal in an INVARIANT position and fixes it exactly, while an
        // extension receiver only requires that the receiver be assignable to the formal's
        // instantiation — a lower bound, not an equation. Unifying it separately pinned `P` to the
