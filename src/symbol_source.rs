...
pub trait SymbolSource {
    ...
    /// Exact singleton classifier generated by the serialization plugin for `classifier`, when the
    /// provider retained that declaration relationship.
    ...
    fn has_serialization_serializer_accessor(
        ...
        self.symbols(SymbolNamespace::Classifier(companion), "serializer")
            .callables
            .functions()
            .iter()
            .any(|function| {
                let serializer = crate::types::type_name("kotlinx/serialization/KSerializer");
                function.callable.params.len() == type_parameters
                    && function.callable.ret.non_null().kotlin_class_internal() == Some(serializer)
                    && function.callable.params.iter().all(|parameter| {
                        parameter.non_null().kotlin_class_internal() == Some(serializer)
                    })
            })
    }
}
