...
use super::{ClassId, FunId, IrPackageProperty, Ty, TypeName};
...
/// Where a module declaration with no dispatch owner is realized. Common lowering fixes it once
/// from the checked declaration: a `companion { … }` block member is a static member of its
/// declaring class, anything else lives in its file's package. A target never recovers the
/// placement from another fact or falls back from one placement to the other.
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum IrStaticPlacement {
    Package,
    CompanionBlock { declaring_class: TypeName },
}
