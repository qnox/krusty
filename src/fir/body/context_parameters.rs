...
impl FirBody {
    ...
    pub fn set_context_parameter_kinds(&mut self, kinds: Vec<crate::types::ContextParameterKind>) {
        assert!(self.context_parameter_kinds.is_empty());
        assert_eq!(kinds.len(), self.context_receiver_types.len());
        self.context_parameter_kinds = kinds;
    }

    pub fn context_parameter_kinds(&self) -> &[crate::types::ContextParameterKind] {
        &self.context_parameter_kinds
    }

    pub fn is_context_value_ordinal(&self, ordinal: usize) -> bool {
        self.context_parameter_kinds.get(ordinal)
            == Some(&crate::types::ContextParameterKind::Named)
    }
