...
mod tests {
    ...
    // --- TParams: build / extend / insert / erase --------------------------------------------

    #[test]
    fn tparams_erased_maps_every_name_to_any() {
        let tp = TParams::erased(&names_of(&["T", "R"]));
        assert!(tp.contains("T"));
        assert!(tp.contains("R"));
        assert!(!tp.contains("Q"));
        assert_eq!(tp.erase("T"), Ty::obj("kotlin/Any"));
        assert_eq!(tp.erase("R"), Ty::obj("kotlin/Any"));
        // An unknown name erases to Any as the documented fallback.
        assert_eq!(tp.erase("Nope"), Ty::obj("kotlin/Any"));
    }

    #[test]
    fn tparams_from_decl_recovers_primitive_bound_only() {
        // Without a resolver: primitive bound recovered, reference bound stays Any.
        let tp = TParams::from_decl(
            &names_of(&["T", "U"]),
            &[
                ("T".to_string(), tref("Int")),
                ("U".to_string(), tref("Widget")),
            ],
        );
        assert_eq!(tp.erase("T"), Ty::Int);
        assert_eq!(tp.erase("U"), Ty::obj("kotlin/Any"));
    }

    #[test]
    fn tparams_from_decl_with_resolves_reference_bound() {
        let resolve = |n: &str| (n == "Widget").then(|| "demo/Widget".to_string());
        let tp = TParams::from_decl_with(
            &names_of(&["T"]),
            &[("T".to_string(), tref("Widget"))],
            &resolve,
        );
        assert_eq!(tp.erase("T"), Ty::obj("demo/Widget"));
    }

    #[test]
    fn tparams_from_decl_unbounded_is_any() {
        let tp = TParams::from_decl(&names_of(&["T"]), &[]);
        assert_eq!(tp.erase("T"), Ty::obj("kotlin/Any"));
    }

    #[test]
    fn tparams_extended_merges_both_scopes() {
        let base = TParams::from_decl(&names_of(&["T"]), &[("T".to_string(), tref("Int"))]);
        let ext = base.extended(&names_of(&["U"]), &[("U".to_string(), tref("String"))]);
        // Both the base and the extension are visible.
        assert_eq!(ext.erase("T"), Ty::Int);
        assert_eq!(ext.erase("U"), Ty::String);
        // The original is unchanged.
        assert!(!base.contains("U"));
    }

    #[test]
    fn tparams_extended_with_uses_resolver() {
        let base = TParams::erased(&names_of(&["T"]));
        let resolve = |n: &str| (n == "Base").then(|| "p/Base".to_string());
        let ext = base.extended_with(
            &names_of(&["U"]),
            &[("U".to_string(), tref("Base"))],
            &resolve,
        );
        assert_eq!(ext.erase("U"), Ty::obj("p/Base"));
    }

    #[test]
    fn tparams_insert_decl_returns_only_newly_added() {
        let mut tp = TParams::erased(&names_of(&["T"]));
        // T already present, U is new → only U reported.
        let added = tp.insert_decl(&names_of(&["T", "U"]), &[("U".to_string(), tref("Int"))]);
        assert_eq!(added, vec!["U".to_string()]);
        // The pre-existing T keeps its original erasure (not overwritten).
        assert_eq!(tp.erase("T"), Ty::obj("kotlin/Any"));
        assert_eq!(tp.erase("U"), Ty::Int);
    }

    #[test]
    fn tparams_insert_decl_with_resolver_and_remove_roundtrip() {
        let mut tp = TParams::default();
        let resolve = |n: &str| (n == "Thing").then(|| "p/Thing".to_string());
        let added = tp.insert_decl_with(
            &names_of(&["T"]),
            &[("T".to_string(), tref("Thing"))],
            &resolve,
        );
        assert_eq!(added, vec!["T".to_string()]);
        assert_eq!(tp.erase("T"), Ty::obj("p/Thing"));
        // Removing exactly the added names restores the empty scope.
        for n in &added {
            tp.remove(n);
        }
        assert!(!tp.contains("T"));
    ...
    #[test]
    fn tparams_from_bindings_is_a_concrete_substitution() {
        let tp =
            TParams::from_bindings([("T".to_string(), Ty::String), ("R".to_string(), Ty::Int)]);
        assert_eq!(tp.erase("T"), Ty::String);
        assert_eq!(tp.erase("R"), Ty::Int);
    }

    #[test]
    fn tparams_clear_empties_the_scope() {
        let mut tp = TParams::erased(&names_of(&["A", "B"]));
        assert!(tp.contains("A"));
        tp.clear();
        assert!(!tp.contains("A"));
        assert!(!tp.contains("B"));
    }
    ...
    // --- deeper fixtures: Checker methods + more pure helpers ---------------------------------

    /// A bare `ClassSig` with every field empty; individual fields are then set per test.
    fn mk_class(internal: &str) -> ClassSig {
        ClassSig {
            ...
            static_methods: HashMap::new(),
            static_props: HashMap::new(),
            lateinit_props: std::collections::HashSet::new(),
            interfaces: Vec::new(),
            super_internal: None,
            is_annotation: false,
            ctor_defaults: Vec::new(),
    ...
    /// The body expression of the first top-level `fun` named `name`.
    fn fn_body(file: &File, name: &str) -> ExprId {
        for &d in &file.decls {
            if let Decl::Fun(f) = file.decl(d) {
                if f.name == name {
                    return match f.body {
                        FunBody::Expr(e) | FunBody::Block(e) => e,
                        FunBody::None => panic!("fn {name} has no body"),
                    };
                }
            }
        }
    ...
    // --- obj_is_subtype: reflexive / interface / super chain / canonical fold / false ---------

    #[test]
    fn obj_is_subtype_reflexive_and_unrelated() {
        let syms = SymbolTable::default();
        let f = parse_file("fun box(): String = \"OK\"");
        let mut d = DiagSink::new();
        let c = make_checker(&f, &syms, &mut d);
        // Reflexive: same internal name.
        assert!(c.obj_is_subtype("demo/A", "demo/A"));
        // Two unrelated, undeclared classpath names: BFS through the (empty) library finds nothing.
        assert!(!c.obj_is_subtype("demo/A", "demo/B"));
    }

    #[test]
    fn obj_is_subtype_walks_interfaces_and_super_chain() {
        let mut syms = SymbolTable::default();
        // Base : implements I; Mid : Base; Leaf : Mid.
        let mut base = mk_class("demo/Base");
        base.interfaces = vec!["demo/I".to_string()];
        let mut mid = mk_class("demo/Mid");
        mid.super_internal = Some("demo/Base".to_string());
        let mut leaf = mk_class("demo/Leaf");
        leaf.super_internal = Some("demo/Mid".to_string());
        syms.classes.insert("Base".into(), base);
        syms.classes.insert("Mid".into(), mid);
        syms.classes.insert("Leaf".into(), leaf);
        let f = parse_file("fun box(): String = \"OK\"");
        let mut d = DiagSink::new();
        let c = make_checker(&f, &syms, &mut d);
        // Direct interface.
        assert!(c.obj_is_subtype("demo/Base", "demo/I"));
        // Multi-level superclass reach + inherited interface.
        assert!(c.obj_is_subtype("demo/Leaf", "demo/Mid"));
        assert!(c.obj_is_subtype("demo/Leaf", "demo/Base"));
        assert!(c.obj_is_subtype("demo/Leaf", "demo/I"));
        // A declared class with a super but no matching ancestor → false (not the classpath BFS).
        assert!(!c.obj_is_subtype("demo/Leaf", "demo/Unrelated"));
    }

    #[test]
    fn obj_is_subtype_folds_canonical_names() {
        let mut syms = SymbolTable::default();
        ...
        syms.canonical_names = std::rc::Rc::new(canon);
        let f = parse_file("fun box(): String = \"OK\"");
        let mut d = DiagSink::new();
        let c = make_checker(&f, &syms, &mut d);
        // Both fold to java/util/List, so they are mutually assignable after the canonical fold.
        assert!(c.obj_is_subtype("kotlin/collections/MutableList", "kotlin/collections/List"));
        assert!(c.obj_is_subtype("kotlin/collections/List", "kotlin/collections/MutableList"));
    }

    #[test]
    fn when_objs_comparable_needs_subtype_relation() {
        let mut syms = SymbolTable::default();
        let mut sub = mk_class("demo/Ok");
        sub.super_internal = Some("demo/Res".to_string());
        syms.classes.insert("Ok".into(), sub);
        syms.classes.insert("Res".into(), mk_class("demo/Res"));
        let f = parse_file("fun box(): String = \"OK\"");
        let mut d = DiagSink::new();
        let c = make_checker(&f, &syms, &mut d);
        // Subtype in either direction is comparable.
        assert!(c.when_objs_comparable(Ty::obj("demo/Res"), Ty::obj("demo/Ok")));
        assert!(c.when_objs_comparable(Ty::obj("demo/Ok"), Ty::obj("demo/Res")));
        // Unrelated references are not comparable; a primitive operand is never an object arm.
        assert!(!c.when_objs_comparable(Ty::obj("demo/Ok"), Ty::obj("demo/Other")));
        assert!(!c.when_objs_comparable(Ty::Int, Ty::obj("demo/Ok")));
    }

    // --- lookup_method / lookup_prop: own + inherited -----------------------------------------

    #[test]
    fn lookup_method_finds_own_interface_and_super() {
        let mut syms = SymbolTable::default();
        let mut iface = mk_class("demo/I");
        iface
            .methods
            .insert("ifm".into(), sig(vec![], Ty::Int, false, 0));
        let mut base = mk_class("demo/Base");
        base.methods
            .insert("bm".into(), sig(vec![], Ty::String, false, 0));
        base.interfaces = vec!["demo/I".to_string()];
        let mut sub = mk_class("demo/Sub");
        sub.super_internal = Some("demo/Base".to_string());
        sub.methods
            .insert("own".into(), sig(vec![Ty::Int], Ty::Int, false, 1));
        syms.classes.insert("I".into(), iface);
        syms.classes.insert("Base".into(), base);
        syms.classes.insert("Sub".into(), sub);
        let f = parse_file("fun box(): String = \"OK\"");
        let mut d = DiagSink::new();
        let c = make_checker(&f, &syms, &mut d);
        assert_eq!(
            c.lookup_method("demo/Sub", "own").map(|s| s.ret),
            Some(Ty::Int)
        );
        // Inherited from the base class.
        assert_eq!(
            c.lookup_method("demo/Sub", "bm").map(|s| s.ret),
            Some(Ty::String)
        );
        // Inherited from an interface implemented by the base.
        assert_eq!(
            c.lookup_method("demo/Sub", "ifm").map(|s| s.ret),
            Some(Ty::Int)
        );
        // Missing everywhere, and an unknown class.
        assert!(c.lookup_method("demo/Sub", "nope").is_none());
        assert!(c.lookup_method("demo/Ghost", "own").is_none());
    }

    #[test]
    fn lookup_prop_walks_super_chain() {
        let mut syms = SymbolTable::default();
        let mut base = mk_class("demo/Base");
        base.props = vec![("count".to_string(), Ty::Int, true)];
        let mut sub = mk_class("demo/Sub");
        sub.super_internal = Some("demo/Base".to_string());
        sub.props = vec![("name".to_string(), Ty::String, false)];
        syms.classes.insert("Base".into(), base);
        syms.classes.insert("Sub".into(), sub);
        let f = parse_file("fun box(): String = \"OK\"");
        let mut d = DiagSink::new();
        let c = make_checker(&f, &syms, &mut d);
        assert_eq!(c.lookup_prop("demo/Sub", "name"), Some((Ty::String, false)));
        assert_eq!(c.lookup_prop("demo/Sub", "count"), Some((Ty::Int, true)));
        assert_eq!(c.lookup_prop("demo/Sub", "missing"), None);
    }

    // --- catch_internal: import / declared class / classpath / none ----------------------------

    #[test]
    fn catch_internal_resolves_import_class_and_none() {
        let mut syms = SymbolTable::default();
        syms.classes.insert("MyErr".into(), mk_class("demo/MyErr"));
        syms.class_names
            .insert("Throwable".into(), "kotlin/Throwable".into());
        let f = parse_file("import java.io.IOException\nfun box(): String = \"OK\"");
        let mut d = DiagSink::new();
        let c = make_checker(&f, &syms, &mut d);
        // An explicit import wins.
        assert_eq!(
            c.catch_internal("IOException"),
            Some("java/io/IOException".to_string())
        );
        // A declared class resolves to its internal name.
        assert_eq!(c.catch_internal("MyErr"), Some("demo/MyErr".to_string()));
        // A class_names (classpath alias) fallback.
        assert_eq!(
            c.catch_internal("Throwable"),
            Some("kotlin/Throwable".to_string())
        );
        // Unknown everywhere.
        assert_eq!(c.catch_internal("Nope"), None);
    }

    // --- resolve_ty_no_diag: primitive / class / function / unresolved ------------------------

    #[test]
    fn resolve_ty_no_diag_covers_leaf_class_and_error() {
        let mut syms = SymbolTable::default();
        syms.classes.insert("Foo".into(), mk_class("demo/Foo"));
        let f = parse_file("fun box(): String = \"OK\"");
        let mut d = DiagSink::new();
        let c = make_checker(&f, &syms, &mut d);
        assert_eq!(c.resolve_ty_no_diag(&tref("Int")), Ty::Int);
        assert_eq!(c.resolve_ty_no_diag(&tref("String")), Ty::String);
        assert_eq!(c.resolve_ty_no_diag(&tref("Foo")), Ty::obj("demo/Foo"));
        assert_eq!(c.resolve_ty_no_diag(&tref("Nope")), Ty::Error);
        // A function type resolves through the recursion, no diagnostics.
        let mut fr = tref("<fun>");
        fr.fun_params = vec![tref("Int")];
        fr.arg = Some(Box::new(tref("String")));
        assert_eq!(
            c.resolve_ty_no_diag(&fr),
            Ty::fun(vec![Ty::Int], Ty::String)
        );
        assert!(d.diags.is_empty(), "no-diag resolver must stay silent");
    }

    // --- body_terminates / stmt_terminates on many shapes -------------------------------------

    fn body_terminates_of(src: &str) -> bool {
        let f = parse_file(src);
        let syms = SymbolTable::default();
        let mut d = DiagSink::new();
        let c = make_checker(&f, &syms, &mut d);
        c.body_terminates(fn_body(&f, "g"))
    }

    #[test]
    fn body_terminates_return_and_throw() {
        assert!(body_terminates_of("fun g(): Int { return 1 }"));
        assert!(body_terminates_of(
            "fun g(): Int = throw RuntimeException()"
        ));
        // A plain expression body does not terminate.
        assert!(!body_terminates_of("fun g(): Int = 1"));
        // An empty block falls through.
        assert!(!body_terminates_of("fun g() { }"));
    }

    #[test]
    fn body_terminates_if_requires_both_branches() {
        assert!(body_terminates_of(
            "fun g(x: Int): Int { if (x > 0) { return 1 } else { return 2 } }"
        ));
        // A one-armed `if` (no else) does not terminate.
        assert!(!body_terminates_of(
            "fun g(x: Int) { if (x > 0) { return } }"
        ));
    }

    #[test]
    fn body_terminates_when_all_arms_and_try() {
        assert!(body_terminates_of(
            "fun g(x: Int): Int = when (x) { 1 -> throw RuntimeException(); else -> throw RuntimeException() }"
        ));
        // A when whose else arm falls through does not terminate.
        assert!(!body_terminates_of(
            "fun g(x: Int): Int = when (x) { 1 -> 1; else -> 2 }"
        ));
        // finally that returns terminates; and body+all-catches terminating terminates.
        assert!(body_terminates_of(
            "fun g(): Int { try { return 1 } finally { return 2 } }"
        ));
        assert!(body_terminates_of(
            "fun g(): Int { try { return 1 } catch (e: Exception) { return 2 } }"
        ));
        // A catch that falls through breaks termination.
        assert!(!body_terminates_of(
            "fun g(): Int { try { return 1 } catch (e: Exception) { 2 } }"
        ));
    }

    #[test]
    fn stmt_terminates_infinite_loops() {
        // `while (true)` / `do while (true)` never fall through.
        assert!(body_terminates_of("fun g() { while (true) { } }"));
        assert!(body_terminates_of("fun g() { do { } while (true) }"));
        // A finite loop condition does not guarantee termination.
        assert!(!body_terminates_of("fun g(n: Int) { while (n > 0) { } }"));
    }

    // --- ty_of_ref / ty_of_ref_with: leaf, class, alias, generic, error arms ------------------

    fn resolve_tref(r: &TypeRef, cn: &ClassNames, tp: &TParams) -> (Ty, Vec<String>) {
        let mut d = DiagSink::new();
        let ty = ty_of_ref(r, cn, tp, &mut d);
        (ty, d.diags.iter().map(|x| x.msg.clone()).collect())
    }

    #[test]
    fn ty_of_ref_primitive_class_and_alias() {
        let mut cn = ClassNames::default();
        cn.insert("Foo".into(), "demo/Foo".into());
        cn.insert("MyInt".into(), "__ty/Int".into());
        let tp = TParams::default();
        assert_eq!(resolve_tref(&tref("Int"), &cn, &tp).0, Ty::Int);
        assert_eq!(resolve_tref(&tref("Foo"), &cn, &tp).0, Ty::obj("demo/Foo"));
        // A `__ty/` alias decodes to the primitive.
        assert_eq!(resolve_tref(&tref("MyInt"), &cn, &tp).0, Ty::Int);
    }

    #[test]
    fn ty_of_ref_generic_instantiation_and_tparam_erasure() {
        let mut cn = ClassNames::default();
        cn.insert("Box".into(), "demo/Box".into());
        let tp = TParams::erased(&["T".to_string()]);
        // `Box<Int>` carries its resolved argument.
        let mut r = tref("Box");
        r.targs = vec![tref("Int")];
        assert_eq!(
            resolve_tref(&r, &cn, &tp).0,
            Ty::obj_args("demo/Box", &[Ty::Int])
        );
        // A type parameter erases to Any.
        assert_eq!(resolve_tref(&tref("T"), &cn, &tp).0, Ty::obj("kotlin/Any"));
    }

    #[test]
    fn ty_of_ref_array_forms() {
        let cn = ClassNames::default();
        let tp = TParams::default();
        // Array<String> → boxed reference array.
        let mut ar = tref("Array");
        ar.arg = Some(Box::new(tref("String")));
        assert_eq!(resolve_tref(&ar, &cn, &tp).0, Ty::array(Ty::String));
        // Array<Int> → boxed Integer array modeled as Obj("kotlin/Array", [Int]).
        let mut ai = tref("Array");
        ai.arg = Some(Box::new(tref("Int")));
        assert_eq!(
            resolve_tref(&ai, &cn, &tp).0,
            Ty::obj_args("kotlin/Array", &[Ty::Int])
        );
        // A raw Array (no element) errors.
        let (ty, errs) = resolve_tref(&tref("Array"), &cn, &tp);
        assert_eq!(ty, Ty::Error);
        assert!(errs.iter().any(|e| e.contains("raw Array")), "{errs:?}");
    }

    #[test]
    fn ty_of_ref_error_and_nullable_arms() {
        let cn = ClassNames::default();
        let tp = TParams::default();
        // Unresolved reference.
        let (ty, errs) = resolve_tref(&tref("Ghost"), &cn, &tp);
        assert_eq!(ty, Ty::Error);
        assert!(errs
            .iter()
            .any(|e| e.contains("unresolved reference 'Ghost'")));
        // KClass without a class-literal ctx errors.
        let (kty, kerrs) = resolve_tref(&tref("KClass"), &cn, &tp);
        assert_eq!(kty, Ty::Error);
        assert!(kerrs.iter().any(|e| e.contains("KClass")));
        // A nullable primitive boxes to Nullable(prim).
        let mut ni = tref("Int");
        ni.nullable = true;
        assert_eq!(resolve_tref(&ni, &cn, &tp).0, Ty::nullable(Ty::Int));
        // `Unit?` is a nullable kotlin/Unit reference.
        let mut nu = tref("Unit");
        nu.nullable = true;
        assert_eq!(
            resolve_tref(&nu, &cn, &tp).0,
            Ty::nullable(Ty::obj("kotlin/Unit"))
        );
    }

    // --- member_signature: params / vararg / defaults / names ---------------------------------

    fn first_method<'a>(f: &'a File, class: &str) -> &'a FunDecl {
        for &d in &f.decls {
            if let Decl::Class(c) = f.decl(d) {
                if c.name == class {
                    return &c.methods[0];
    ...
    #[test]
    fn member_signature_maps_params_and_defaults() {
        let f = parse_file("class C { fun m(a: Int, b: String = \"x\"): Int = a }");
        let m = first_method(&f, "C");
        let mut d = DiagSink::new();
        let s = member_signature(
            m,
            Ty::Int,
            &ClassNames::default(),
            &TParams::default(),
        ...
        assert_eq!(s.params, vec![Ty::Int, Ty::String]);
        assert_eq!(s.ret, Ty::Int);
        assert_eq!(s.required, 1);
        assert!(!s.vararg);
        assert_eq!(s.param_defaults, vec![false, true]);
        assert_eq!(s.param_names, vec!["a".to_string(), "b".to_string()]);
    }

    #[test]
    fn member_signature_vararg_wraps_last_param_in_array() {
        let f = parse_file("class C { fun m(vararg xs: Int) {} }");
        let m = first_method(&f, "C");
        let mut d = DiagSink::new();
        let s = member_signature(
            m,
        ...
        assert_eq!(s.params, vec![Ty::array(Ty::Int)]);
        assert!(s.vararg);
    }

    // --- infer_getter_ty: literals / names / member / unary / binary --------------------------

    #[test]
    fn infer_getter_ty_literals_and_ops() {
        let mut locals: HashMap<&str, Ty> = HashMap::new();
        locals.insert("x", Ty::Long);
        locals.insert("count", Ty::Int);
        let f = parse_file(
            "fun a(): Int = 1 + 2\n\
             fun b(): Boolean = 1 < 2\n\
             fun c(): String = \"x\" + 1\n\
        ...
        assert_eq!(infer_getter_ty(&f, fn_body(&f, "a"), &locals), Ty::Int);
        assert_eq!(infer_getter_ty(&f, fn_body(&f, "b"), &locals), Ty::Boolean);
        assert_eq!(infer_getter_ty(&f, fn_body(&f, "c"), &locals), Ty::String);
        assert_eq!(infer_getter_ty(&f, fn_body(&f, "d"), &locals), Ty::Long);
        assert_eq!(infer_getter_ty(&f, fn_body(&f, "e"), &locals), Ty::Int);
        assert_eq!(infer_getter_ty(&f, fn_body(&f, "g"), &locals), Ty::Int);
        // An unknown call yields Error.
        assert_eq!(infer_getter_ty(&f, fn_body(&f, "h"), &locals), Ty::Error);
    }

    #[test]
    fn infer_getter_ty_unary_not_is_boolean() {
        let locals: HashMap<&str, Ty> = HashMap::new();
        let f = parse_file("fun g(): Boolean = !(1 < 2)\nfun n(): Int = -5");
        assert_eq!(infer_getter_ty(&f, fn_body(&f, "g"), &locals), Ty::Boolean);
        assert_eq!(infer_getter_ty(&f, fn_body(&f, "n"), &locals), Ty::Int);
    }

    // --- infer_lit_ty: literals / binary / unresolved -----------------------------------------

    fn infer_lit_of(src: &str, name: &str) -> Ty {
        let f = parse_file(src);
        let cn = ClassNames::default();
        let fun_rets: HashMap<String, Ty> = HashMap::new();
        infer_lit_ty(&f, fn_body(&f, name), &cn, &fun_rets, &EmptySymbolSource)
    }

    #[test]
    fn infer_lit_ty_literal_and_binary_arms() {
        assert_eq!(infer_lit_of("fun g() = 1L", "g"), Ty::Long);
        assert_eq!(infer_lit_of("fun g() = 1.5", "g"), Ty::Double);
        assert_eq!(infer_lit_of("fun g() = \"s\"", "g"), Ty::String);
        assert_eq!(infer_lit_of("fun g() = 't'", "g"), Ty::Char);
        assert_eq!(infer_lit_of("fun g() = true && false", "g"), Ty::Boolean);
        assert_eq!(infer_lit_of("fun g() = 1 + 2", "g"), Ty::Int);
        assert_eq!(infer_lit_of("fun g() = \"a\" + 1", "g"), Ty::String);
        // A bare name that resolves to nothing (no props, EmptySymbolSource) → Error.
        assert_eq!(infer_lit_of("fun g() = mystery", "g"), Ty::Error);
        // A call to an unknown callee → Error.
        assert_eq!(infer_lit_of("fun g() = mystery()", "g"), Ty::Error);
    }

    // --- expr_uses_name / expr_uses_name_deep: capture detection ------------------------------

    #[test]
    fn expr_uses_name_shallow_stops_at_lambda_but_deep_recurses() {
        let f = parse_file("fun g(a: Int): Int { val f = { a + 1 }; return 0 }");
        // Find the lambda expression via a scan of the arena.
        let mut lambda: Option<ExprId> = None;
        for e in 0..f.expr_arena.len() {
            let id = ExprId(e as u32);
            if matches!(f.expr(id), Expr::Lambda { .. }) {
                lambda = Some(id);
            }
        }
        let lam = lambda.expect("expected a lambda");
        // The lambda body uses `a`; the deep walk (into the lambda) sees it.
        assert!(expr_uses_name_deep(&f, lam, "a"));
        assert!(!expr_uses_name_deep(&f, lam, "zzz"));
    }

    #[test]
    fn expr_uses_name_pub_direct_reference() {
        let f = parse_file("fun g(a: Int): Int = a + 1");
        let body = fn_body(&f, "g");
        assert!(expr_uses_name_pub(&f, body, "a"));
        assert!(!expr_uses_name_pub(&f, body, "b"));
    }

    // --- expr_has_try / expr_has_finally family -----------------------------------------------

    #[test]
    fn expr_try_predicates_detect_nested_and_finally() {
        let f = parse_file(
            "fun g(): Int { try { return try { 1 } finally { 2 } } finally { return 3 } }",
        );
        let body = fn_body(&f, "g");
        assert!(expr_has_try(&f, body));
        assert!(expr_has_finally(&f, body));
        // The outer finally has a `return` in the try body that crosses it.
        assert!(expr_try_finally_has_return(&f, body));
        assert!(expr_has_return(&f, body));
    }

    #[test]
    fn expr_try_predicates_negative_and_finally_with_try() {
        // No try at all.
        let plain = parse_file("fun g(): Int = 1 + 2");
        let pb = fn_body(&plain, "g");
        assert!(!expr_has_try(&plain, pb));
        assert!(!expr_has_finally(&plain, pb));
        assert!(!expr_has_return(&plain, pb));
        // A finally that itself contains a try.
        let ft =
            parse_file("fun g() { try { 1 } finally { try { 2 } catch (e: Exception) { 3 } } }");
        let fb = fn_body(&ft, "g");
        assert!(expr_has_finally_with_try(&ft, fb));
        // A finally without an inner try does not trip the predicate.
        let nof = parse_file("fun g() { try { 1 } finally { 2 } }");
        let nb = fn_body(&nof, "g");
        assert!(!expr_has_finally_with_try(&nof, nb));
    }

    // --- bc_complex_e: break/continue position analysis ---------------------------------------

    fn bc_complex_of(src: &str) -> bool {
        let f = parse_file(src);
        bc_complex_e(&f, fn_body(&f, "g"), false, false)
    }

    #[test]
    fn bc_complex_break_positions() {
        // A break in plain statement position inside a loop is a simple goto — not complex.
        assert!(!bc_complex_of("fun g() { while (true) { break } }"));
        // A break as a direct `if` branch inside a loop is fine.
        assert!(!bc_complex_of(
            "fun g(x: Int) { while (true) { if (x > 0) break } }"
        ));
        // A break inside a lambda (non-local jump) is complex.
        assert!(bc_complex_of("fun g() { while (true) { run { break } } }"));
        // A break inside a `try` region is complex.
        assert!(bc_complex_of(
            "fun g() { while (true) { try { break } finally { } } }"
        ));
    }

    // --- collect_file_type_names: params / return / supertypes / value names -------------------

    #[test]
    fn collect_file_type_names_gathers_refs_and_value_names() {
        let f = parse_file(
            "fun helper(p: Widget): Gadget = something\n\
             class Impl : Base, Iface",
        );
        let mut out = std::collections::HashSet::new();
        collect_file_type_names(&f, &mut out);
        assert!(out.contains("Widget"));
        assert!(out.contains("Gadget"));
        // A bare value name is over-approximated in.
        assert!(out.contains("something"));
        // Supertypes/base names are collected.
        assert!(out.contains("Base"));
        assert!(out.contains("Iface"));
    }

    // --- resolve_dotted_classpath_type / resolve_nested_internal: None under empty source ------

    #[test]
    fn resolve_dotted_and_nested_none_under_empty_source() {
        // EmptySymbolSource resolves nothing, so a dotted name never becomes a phantom internal.
        assert_eq!(
            resolve_dotted_classpath_type(
                "a.b.Missing",
                &ClassNames::default(),
        ...
        // A non-dotted name is rejected up front.
        assert_eq!(
            resolve_dotted_classpath_type(
                "Plain",
                &ClassNames::default(),
        ...
        assert_eq!(
            resolve_nested_internal("a/b/Outer/Inner", &EmptySymbolSource),
            None
        );
    }
}
