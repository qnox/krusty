...
mod tests {
    ...
    #[test]
    fn boxed_ref_covers_every_signed_primitive_and_rejects_references() {
        // Each signed JVM primitive boxes to its Kotlin wrapper name.
        assert_eq!(Ty::Byte.boxed_ref(), Some(Ty::obj("kotlin/Byte")));
        assert_eq!(Ty::Short.boxed_ref(), Some(Ty::obj("kotlin/Short")));
        assert_eq!(Ty::Long.boxed_ref(), Some(Ty::obj("kotlin/Long")));
        assert_eq!(Ty::Float.boxed_ref(), Some(Ty::obj("kotlin/Float")));
        assert_eq!(Ty::Double.boxed_ref(), Some(Ty::obj("kotlin/Double")));
        assert_eq!(Ty::Boolean.boxed_ref(), Some(Ty::obj("kotlin/Boolean")));
        assert_eq!(Ty::ULong.boxed_ref(), Some(Ty::obj("kotlin/ULong")));
        // Non-primitive shapes have no boxed form.
        assert_eq!(Ty::Unit.boxed_ref(), None);
        assert_eq!(Ty::Null.boxed_ref(), None);
        assert_eq!(Ty::obj("demo/Point").boxed_ref(), None);
    }

    #[test]
    fn unboxed_primitive_covers_every_signed_wrapper() {
        assert_eq!(Ty::obj("kotlin/Byte").unboxed_primitive(), Some(Ty::Byte));
        assert_eq!(Ty::obj("kotlin/Short").unboxed_primitive(), Some(Ty::Short));
        assert_eq!(Ty::obj("kotlin/Long").unboxed_primitive(), Some(Ty::Long));
        assert_eq!(Ty::obj("kotlin/Float").unboxed_primitive(), Some(Ty::Float));
        assert_eq!(
            Ty::obj("kotlin/Double").unboxed_primitive(),
            Some(Ty::Double)
        );
        assert_eq!(
            Ty::obj("kotlin/Boolean").unboxed_primitive(),
            Some(Ty::Boolean)
        );
        // ULong is not a signed wrapper, so it does not unbox here.
        assert_eq!(Ty::obj("kotlin/ULong").unboxed_primitive(), None);
    }

    #[test]
    fn erased_recv_folds_nullable_ulong_and_boxes_other_signed_primitives() {
        // A nullable unsigned long boxes to its own wrapper.
        assert_eq!(
            Ty::nullable(Ty::ULong).erased_recv(),
            Ty::obj("kotlin/ULong")
        );
        // Other nullable signed primitives box to their wrapper class.
        assert_eq!(Ty::nullable(Ty::Long).erased_recv(), Ty::obj("kotlin/Long"));
        assert_eq!(
            Ty::nullable(Ty::Double).erased_recv(),
            Ty::obj("kotlin/Double")
        );
        assert_eq!(
            Ty::nullable(Ty::Boolean).erased_recv(),
            Ty::obj("kotlin/Boolean")
        );
        // A function receiver stays distinct by full signature (not merged to a plain FunctionN key).
        let f = Ty::fun(vec![Ty::Int], Ty::Int);
        assert_eq!(f.erased_recv(), f);
        // A bare primitive is its own key.
        assert_eq!(Ty::Char.erased_recv(), Ty::Char);
    }

    #[test]
    fn from_name_covers_every_scalar_name() {
        assert_eq!(Ty::from_name("Byte"), Some(Ty::Byte));
        assert_eq!(Ty::from_name("Short"), Some(Ty::Short));
        assert_eq!(Ty::from_name("Long"), Some(Ty::Long));
        assert_eq!(Ty::from_name("Float"), Some(Ty::Float));
        assert_eq!(Ty::from_name("Double"), Some(Ty::Double));
        assert_eq!(Ty::from_name("Boolean"), Some(Ty::Boolean));
        assert_eq!(Ty::from_name("Char"), Some(Ty::Char));
        assert_eq!(Ty::from_name("UInt"), Some(Ty::UInt));
        assert_eq!(Ty::from_name("String"), Some(Ty::String));
        assert_eq!(Ty::from_name(""), None);
    }

    #[test]
    fn primitive_array_element_covers_all_remaining_names() {
        assert_eq!(Ty::primitive_array_element("LongArray"), Some(Ty::Long));
        assert_eq!(Ty::primitive_array_element("DoubleArray"), Some(Ty::Double));
        assert_eq!(Ty::primitive_array_element("FloatArray"), Some(Ty::Float));
        assert_eq!(
            Ty::primitive_array_element("BooleanArray"),
            Some(Ty::Boolean)
        );
        assert_eq!(Ty::primitive_array_element("ShortArray"), Some(Ty::Short));
        assert_eq!(Ty::primitive_array_element(""), None);
    }

    #[test]
    fn kotlin_class_internal_covers_remaining_primitive_arms() {
        assert_eq!(Ty::Byte.kotlin_class_internal(), Some("kotlin/Byte"));
        assert_eq!(Ty::Short.kotlin_class_internal(), Some("kotlin/Short"));
        assert_eq!(Ty::Long.kotlin_class_internal(), Some("kotlin/Long"));
        assert_eq!(Ty::Char.kotlin_class_internal(), Some("kotlin/Char"));
        assert_eq!(Ty::Float.kotlin_class_internal(), Some("kotlin/Float"));
        assert_eq!(Ty::Double.kotlin_class_internal(), Some("kotlin/Double"));
        assert_eq!(Ty::Boolean.kotlin_class_internal(), Some("kotlin/Boolean"));
        assert_eq!(Ty::ULong.kotlin_class_internal(), Some("kotlin/ULong"));
        assert_eq!(Ty::UInt.kotlin_class_internal(), Some("kotlin/UInt"));
        // Unit / array / function have no source class identity here.
        assert_eq!(Ty::Unit.kotlin_class_internal(), None);
        assert_eq!(Ty::array(Ty::Int).kotlin_class_internal(), None);
    }
