...
mod tests {
    ...
    // --- Arity / lambda-index helpers -------------------------------------------------------

    #[test]
    fn default_omit_indices_identity_when_counts_match() {
        // Equal counts → identity mapping, for arity 0 and arity N.
        assert_eq!(
            default_omit_lambda_param_indices(0, &[]),
            Some(Vec::<usize>::new())
        );
        assert_eq!(
            default_omit_lambda_param_indices(2, &[Some(Ty::Int), Some(Ty::String)]),
            Some(vec![0, 1])
        );
    }

    #[test]
    fn default_omit_indices_maps_trailing_lambda_to_last_param() {
        // A sole trailing lambda (`runBlocking { … }`): the one `None` arg maps to the LAST param.
        assert_eq!(default_omit_lambda_param_indices(3, &[None]), Some(vec![2]));
        // Leading provided args keep their positions; the trailing lambda jumps to the last param.
        assert_eq!(
            default_omit_lambda_param_indices(4, &[Some(Ty::Int), None]),
            Some(vec![0, 3])
        );
    }

    #[test]
    fn default_omit_indices_none_for_unalignable_shapes() {
        // More params than args but the trailing arg is NOT an omitted lambda (`Some`) → no alignment.
        assert!(default_omit_lambda_param_indices(3, &[Some(Ty::Int)]).is_none());
        // Fewer params than args → no alignment.
        assert!(default_omit_lambda_param_indices(1, &[Some(Ty::Int), Some(Ty::Int)]).is_none());
        // Zero args but params expected → n < 1, no trailing lambda to place.
        assert!(default_omit_lambda_param_indices(2, &[]).is_none());
    }

    // --- Lambda-argument / function-parameter arity matching --------------------------------

    #[test]
    fn fun_arg_matches_rejects_non_lambda_argument() {
        // A non-function argument never matches a function-typed parameter.
        assert!(!fun_arg_matches(&Ty::fun(vec![], Ty::Unit), &Ty::Int));
        assert!(!fun_arg_matches(
            &Ty::obj("kotlin/jvm/functions/Function0"),
            &Ty::String
        ));
    }

    #[test]
    fn fun_arg_matches_on_decoded_fun_parameter_by_arity() {
        let lambda = Ty::fun(vec![Ty::Int], Ty::Unit); // arity 1
                                                       // A decoded `Ty::Fun` parameter matches purely on arity (bodies adapt).
        assert!(fun_arg_matches(
            &Ty::fun(vec![Ty::String], Ty::Boolean),
            &lambda
        ));
        // Different arity does not match.
        assert!(!fun_arg_matches(&Ty::fun(vec![], Ty::Boolean), &lambda));
    }

    #[test]
    fn fun_arg_matches_through_nullable_fun_parameter() {
        let lambda = Ty::fun(vec![Ty::Int], Ty::Unit); // arity 1
                                                       // A nullable function parameter unwraps to its inner function type before matching.
        assert!(fun_arg_matches(
            &Ty::nullable(Ty::fun(vec![Ty::String], Ty::Unit)),
            &lambda
        ));
    }

    #[test]
    fn fun_arg_matches_on_erased_functionn_object() {
        let lambda = Ty::fun(vec![Ty::Int], Ty::Unit); // arity 1
                                                       // An erased `FunctionN` object parameter matches when N equals the lambda arity.
        assert!(fun_arg_matches(
        ...
        // A non-function object parameter never matches.
        assert!(!fun_arg_matches(&Ty::obj("demo/Box"), &lambda));
    }

    // --- Pure member-overload selection -----------------------------------------------------

    fn member_overload_info(name: &str, params: Vec<Ty>) -> FunctionInfo {
        let callable = LibraryCallable {
            owner: "demo/Box".to_string(),
            name: name.to_string(),
            params,
    ...
    #[test]
    fn best_member_overload_prefers_exact_params() {
        let exact = member_overload_info("m", vec![Ty::Int]);
        let widen = member_overload_info("m_any", vec![Ty::obj("kotlin/Any")]);
        let cands = [widen, exact];
        let pick = best_member_overload(cands.iter(), "m", &[Ty::Int])
            .expect("an exact-arity match should be found");
        assert_eq!(pick.callable.name, "m");
    }

    #[test]
    fn best_member_overload_widens_reference_to_any() {
        let widen = member_overload_info("m_any", vec![Ty::obj("kotlin/Any")]);
        let cands = [widen];
        let pick = best_member_overload(cands.iter(), "m", &[Ty::String])
            .expect("an `Any` parameter accepts any reference argument");
        assert_eq!(pick.callable.name, "m_any");
    }

    #[test]
    fn best_member_overload_matches_arg_prefix_of_longer_params() {
        // No exact/equal-arity candidate; the args form a prefix of a longer parameter list.
        let longer = member_overload_info("m_defaulted", vec![Ty::Int, Ty::Int]);
        let cands = [longer];
        let pick = best_member_overload(cands.iter(), "m", &[Ty::Int])
            .expect("args are a matching prefix of the longer signature");
        assert_eq!(pick.callable.name, "m_defaulted");
    }

    #[test]
    fn best_member_overload_none_when_nothing_fits() {
        let mismatch = member_overload_info("m", vec![Ty::String]);
        let cands = [mismatch];
