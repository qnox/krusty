...
fn collect(dir: &Path, out: &mut Vec<PathBuf>) {
    if let Ok(rd) = std::fs::read_dir(dir) {
        for e in rd.flatten() {
            ...
            else if p.extension().map_or(false, |x| x == "kt") { out.push(p); }
        }
    }
}
...
fn main() {
    ...
    for f in &files {
        ...
        if src.contains("// FILE:") || src.contains("// MODULE:") || !src.contains("fun box()") { continue; }
        scanned += 1;
        ...
        if d.has_errors() { continue; }
        frontend_ok += 1;
        if lower_file(&files1[0], &info, &syms).is_some() { lowered += 1; }
    }
    eprintln!("DONE scanned={scanned} frontend_ok={frontend_ok} lowered={lowered} bailed={}", frontend_ok - lowered);
}
