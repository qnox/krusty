...
mod tests {
    ...
    #[test]
    fn declares_top_level_is_a_presence_predicate() {
        let mut st = SymbolTable::default();
        st.funs.insert("mine".into(), vec![sig(vec![], Ty::Unit)]);
        let m = ModuleSymbols::new(&st);
        assert!(m.declares_top_level("mine"));
        assert!(!m.declares_top_level("absent"));
    }

    #[test]
    fn resolve_top_level_picks_the_matching_overload() {
        let mut st = SymbolTable::default();
        st.funs.insert(
            "f".into(),
            vec![sig(vec![Ty::Int], Ty::Int), sig(vec![Ty::String], Ty::Long)],
        );
        let m = ModuleSymbols::new(&st);
        // Int arg selects the Int overload (ret Int); String arg selects the other (ret Long).
        let by_int = m.resolve_top_level("f", &[Ty::Int]).expect("int overload");
        assert_eq!(by_int.callable.ret, Ty::Int);
        let by_str = m
            .resolve_top_level("f", &[Ty::obj("kotlin/String")])
            .expect("string overload");
        assert_eq!(by_str.callable.ret, Ty::Long);
        // No overload accepts these args, and an unknown name resolves to nothing.
        assert!(m
            .resolve_top_level("f", &[Ty::Boolean, Ty::Boolean])
            .is_none());
        assert!(m.resolve_top_level("absent", &[Ty::Int]).is_none());
    }

    #[test]
    fn members_visit_interfaces_before_superclass() {
        // Pre-order self → interface subtree → superclass subtree: an interface method outranks a
        // superclass method of the same name (the checker's lookup_method order).
        let mut st = SymbolTable::default();
        let mut iface = class("demo/Iface");
        iface.is_interface = true;
        iface.methods.insert("m".into(), sig(vec![], Ty::Int));
        let mut base = class("demo/Base");
        base.methods.insert("m".into(), sig(vec![], Ty::String));
        let mut sub = class("demo/Sub");
        sub.interfaces = vec!["demo/Iface".into()];
        sub.super_internal = Some("demo/Base".into());
        st.classes.insert("Iface".into(), iface);
        st.classes.insert("Base".into(), base);
        st.classes.insert("Sub".into(), sub);
        let m = ModuleSymbols::new(&st);
        let fs = m.functions("m", Some(Ty::obj("demo/Sub")));
        assert_eq!(fs.overloads.len(), 2);
        // Self (Sub) consumes rung 0; interface visited before superclass, so Iface precedes Base.
        assert_eq!(fs.overloads[0].callable.owner, "demo/Iface");
        assert_eq!(fs.overloads[1].callable.owner, "demo/Base");
        assert!(fs.overloads[0].receiver_rank < fs.overloads[1].receiver_rank);
    }

    #[test]
    fn extension_falls_back_to_generic_any_receiver() {
        // A type-variable-receiver extension registered under the `Any` key is found for any receiver
        // (rung 1), when no exact-receiver extension exists.
        let mut st = SymbolTable::default();
        st.ext_funs.insert(
            (Ty::obj("kotlin/Any"), "dump".into()),
            sig(vec![], Ty::Unit),
        );
        let m = ModuleSymbols::new(&st);
        let recv = Ty::obj("demo/Widget");
        let fs = m.functions("dump", Some(recv));
        assert_eq!(fs.overloads.len(), 1);
        assert_eq!(fs.overloads[0].kind, FnKind::Extension);
        assert_eq!(fs.overloads[0].receiver_rank, 1); // Any fallback rung
    }

    #[test]
    fn functions_with_receiver_returns_empty_for_no_member_or_extension() {
        let st = SymbolTable::default();
        let m = ModuleSymbols::new(&st);
        let fs = m.functions("ghost", Some(Ty::obj("demo/Nothing")));
        assert!(fs.overloads.is_empty());
    }

    #[test]
    fn resolve_type_reports_interface_and_annotation_kinds() {
        let mut st = SymbolTable::default();
        let mut iface = class("demo/Shape");
        iface.is_interface = true;
        st.classes.insert("Shape".into(), iface);
        let mut ann = class("demo/Marker");
        ann.is_annotation = true;
        st.classes.insert("Marker".into(), ann);
        st.classes.insert("Plain".into(), class("demo/Plain"));
        let m = ModuleSymbols::new(&st);
        assert!(matches!(
            m.resolve_type("demo/Shape").unwrap().kind,
            crate::libraries::TypeKind::Interface
        ));
        assert!(matches!(
            m.resolve_type("demo/Marker").unwrap().kind,
            crate::libraries::TypeKind::Annotation
        ));
        assert!(matches!(
            m.resolve_type("demo/Plain").unwrap().kind,
            crate::libraries::TypeKind::Class
        ));
    }

    #[test]
    fn resolve_type_carries_secondary_ctors_and_companion_members() {
        let mut st = SymbolTable::default();
        let mut c = class("demo/Box");
        c.ctor_params = vec![Ty::Int];
        c.secondary_ctors = vec![vec![Ty::Int, Ty::Int], vec![]];
        c.static_methods
            .insert("of".into(), sig(vec![Ty::Int], Ty::obj("demo/Box")));
        c.super_internal = Some("demo/Base".into());
        st.classes.insert("Box".into(), c);
        let m = ModuleSymbols::new(&st);
        let t = m.resolve_type("demo/Box").unwrap();
        // Primary + two secondaries, all `<init>` returning Unit.
        assert_eq!(t.constructors.len(), 3);
        assert!(t
            .constructors
            .iter()
            .all(|k| k.name == "<init>" && k.ret == Ty::Unit));
        assert_eq!(t.constructors[0].params, vec![Ty::Int]);
        assert_eq!(t.constructors[1].params, vec![Ty::Int, Ty::Int]);
        assert!(t.constructors[2].params.is_empty());
        // Companion (static) members surface separately.
        assert_eq!(t.companion.len(), 1);
        assert_eq!(t.companion[0].name, "of");
        // Superclass appended after interfaces in supertypes.
        assert_eq!(t.supertypes, vec!["demo/Base".to_string()]);
    }

    #[test]
    fn seed_includes_objects_and_enums_not_just_classes() {
        let mut st = SymbolTable::default();
        st.objects.insert("Singleton".into());
        st.enums
            .insert("Color".into(), vec!["RED".into(), "GREEN".into()]);
        st.class_names
            .insert("Singleton".into(), "demo/Singleton".into());
        st.class_names.insert("Color".into(), "demo/Color".into());
        // Declared but with no recorded internal name → skipped.
        st.objects.insert("Orphan".into());
        let m = ModuleSymbols::new(&st);
        let seed = m.seed();
        assert_eq!(
            seed.class_names.get("Singleton"),
            Some(&"demo/Singleton".into())
        );
        assert_eq!(seed.class_names.get("Color"), Some(&"demo/Color".into()));
        assert!(!seed.class_names.contains_key("Orphan"));
    }
}
