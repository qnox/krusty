...
/// (no setter) is left at its initializer. Yields the constructed object (bound at `obj_slot`).
fn construct_obj(
    ir: &mut IrFile,
    foo_id: u32,
    fields: &[(String, Ty)],
    ...
    let class_internal = ir.classes[foo_id as usize].fq_name.clone();
    let n_ctor = n_ctor.min(fields.len());
    let new = ir.add_expr(IrExpr::New {
        class: foo_id,
        args: values[..n_ctor].to_vec(),
        ctor_params: None,
    });
    // Common case — every serializable property is a constructor parameter: `new Foo(v0, …)` directly,
    // no temporary (keeps the generated bytecode identical to before this body-property support).
    if n_ctor == fields.len() {
        return new;
    }
    let mut stmts = vec![ir.add_expr(IrExpr::Variable {
        index: obj_slot,
        ty: class_ty(&class_internal),
        init: Some(new),
    })];
    for k in n_ctor..fields.len() {
        ...
        let setcall = ir.add_expr(IrExpr::Call {
            callee: Callee::Virtual {
                owner: class_internal.clone(),
                name: sname,
                descriptor: format!("({})V", ty_descriptor(&fields[k].1)),
                interface: false,
            },
    ...
    let read = ir.add_expr(IrExpr::GetValue(obj_slot));
                ...
                match ir.functions[fid as usize].name.as_str() {
                    ...
                    "deserialize" => {
                        ...
                        let body = if decodable {
                            ...
                            // return Foo(f0, …) — the leading `ctor_param_count` decoded values are
                            // primary-ctor args; trailing BODY properties (`var`, incl. inherited) are set
                            // via their setters (`construct_obj`). `next` is the free local for the object.
                            let n_ctor = ir.classes[foo_id as usize].ctor_param_count as usize;
                            let values: Vec<ExprId> = (0..fields.len())
                                .map(|k| ir.add_expr(IrExpr::GetValue(slots[k])))
                                .collect();
                            let new = construct_obj(ir, foo_id, &fields, n_ctor, &values, next);
                            ...
                            // Body properties are set via setters too (so a body-prop class still builds).
                            let n_ctor = ir.classes[foo_id as usize].ctor_param_count as usize;
                            let values: Vec<ExprId> = field_types
                                .iter()
                                .map(|ty| {
                                ...
                                .collect();
                            let new = construct_obj(ir, foo_id, &fields, n_ctor, &values, 2);
                            let ret = ir.add_expr(IrExpr::Return(Some(new)));
                            ir.add_expr(IrExpr::Block {
                                stmts: vec![ret],
                                value: None,
