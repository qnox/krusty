...
impl BodyLowering<'_> {
    ...
    fn iterator_loop(
        ...
        // Kotlin's desugaring calls `iterator()` on the loop subject directly: only the iterator is a
        // loop-scoped temporary, so the subject occupies no local slot of its own.
        let iterable_value = self.expression(iterable)?;
