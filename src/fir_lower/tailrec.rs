...
fn tail_value(
    ...
    match ir.expr(expression).clone() {
        ...
        // after it. `tail_value` descends into blocks, `when` branches and returns alone, so a call
        // guarded by a `try`, a loop or any other construct is left as the ordinary call it is.
        IrExpr::Block {
            mut stmts,
            value: None,
        } if result == Ty::Unit
