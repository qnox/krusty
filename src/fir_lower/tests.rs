...
/// A read of a constructor's synthetic prefix parameter carries WHICH of the two lookups it is.
/// These two pin that lowering performs exactly that one, and fails when it cannot — the contract
/// that replaced a search over every capture of the same owner and field plus a reinterpretation of
/// the node as the constructor's own parameter when that search missed.
#[test]
