//! Checked KLIB wire metadata decoding.
//!
...
pub(crate) struct KlibModuleHeader {
    ...
    pub(crate) package_fragment_names: Vec<String>,
}

pub(crate) struct DecodedPackageFragment<'a> {
    pub(crate) strings: Vec<String>,
    pub(crate) qnames: Vec<QName>,
    pub(crate) package: Option<&'a [u8]>,
...
/// a semantic adapter decides how to represent the resulting name.
...
fn parse_qname(body: &[u8], base: usize) -> Result<QName, PackageFragmentDecodeError> {
    ...
    while !cursor.at_end() {
        let (number, wire) = field(&mut cursor, "qualified-name entry")?;
        match (number, wire) {
            (1, 0) => qname.parent = cursor.varint("qualified-name parent")? as i64,
            (2, 0) => {
                let short = cursor.varint("qualified-name short-name id")?;
                qname.short = usize::try_from(short).map_err(|_| PackageFragmentDecodeError {
                    offset: base,
                    detail: format!("qualified-name short-name id {short} exceeds host range"),
                })?;
            }
            (3, 0) => qname.kind = cursor.varint("qualified-name kind")?,
            (_, wire) => cursor.skip(wire, "qualified-name entry")?,
        }
    }
    Ok(qname)
}
...
fn decode_qualified_names(
    ...
    while cursor.offset < bytes.len() {
        ...
        if number == 1 {
            ...
            qnames.push(parse_qname(entry, entry_base)?);
        } else {
...
pub(crate) fn decode_package_fragment(
    bytes: &[u8],
