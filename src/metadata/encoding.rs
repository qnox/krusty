...
mod tests {
    ...
    #[test]
    fn bytes_to_strings_roundtrips_all_byte_values() {
        // Every byte 0..=255 must survive byte→char→byte identity.
        let bytes: Vec<u8> = (0u16..=255).map(|b| b as u8).collect();
        let strings = bytes_to_strings(&bytes);
        assert_eq!(strings.len(), 1, "should fit one chunk");
        assert_eq!(strings_to_bytes(&strings), bytes);
    }

    #[test]
    fn bytes_to_strings_empty_is_single_empty_chunk() {
        let strings = bytes_to_strings(&[]);
        assert_eq!(strings, vec![String::new()]);
        assert!(strings_to_bytes(&strings).is_empty());
    }

    #[test]
    fn bytes_to_strings_splits_at_utf8_length_limit() {
        // ASCII bytes are one modified-UTF-8 byte each, so a payload just over the 65535 limit
        // must split into two chunks, and still round-trip exactly.
        let bytes = vec![b'A'; MAX_UTF8_INFO_LENGTH + 10];
        let strings = bytes_to_strings(&bytes);
        assert_eq!(strings.len(), 2);
        assert_eq!(strings[0].chars().count(), MAX_UTF8_INFO_LENGTH);
        assert_eq!(strings[1].chars().count(), 10);
        assert_eq!(strings_to_bytes(&strings), bytes);
    }

    #[test]
    fn two_byte_chars_split_earlier() {
        // A NUL byte costs two modified-UTF-8 bytes, so the chunk fills after half as many.
        let bytes = vec![0x00u8; MAX_UTF8_INFO_LENGTH]; // 2 * that many UTF-8 bytes
        let strings = bytes_to_strings(&bytes);
    ...
    #[test]
    fn utf8_width_covers_all_ranges() {
        assert_eq!(utf8_width(0x0000), 2); // NUL is 2 bytes in modified UTF-8
        assert_eq!(utf8_width(0x0001), 1);
        assert_eq!(utf8_width(0x007f), 1);
        assert_eq!(utf8_width(0x0080), 2);
        assert_eq!(utf8_width(0x07ff), 2);
        assert_eq!(utf8_width(0x0800), 3);
        assert_eq!(utf8_width(0xffff), 3);
        assert_eq!(utf8_width(0x1_0000), 3);
    }

    #[test]
    fn modified_utf8_three_byte_range() {
        // U+0800 -> E0 A0 80 (first 3-byte code point).
        assert_eq!(modified_utf8("\u{0800}"), vec![0xe0, 0xa0, 0x80]);
        // U+FFFF -> EF BF BF.
        assert_eq!(modified_utf8("\u{ffff}"), vec![0xef, 0xbf, 0xbf]);
    }

    #[test]
    fn modified_utf8_supplementary_is_surrogate_pair() {
        // U+1F600 encodes as a CESU-8 surrogate pair, six bytes total.
        let enc = modified_utf8("\u{1F600}");
        assert_eq!(enc.len(), 6);
    ...
    #[test]
    fn modified_utf8_empty_string() {
        assert!(modified_utf8("").is_empty());
    }
}
