...
mod compiles_against_the_klib {
    ...
    // The end-to-end test that compiled a program THROUGH this provider lands with the code
    // generator: it drives `CraneliftBackend`, which is not in the crate yet. The five tests
    // above are the provider's own — they read a real Kotlin/Native distribution rather than a
    // fixture, which is what keeps them from being self-consistency checks.
}
