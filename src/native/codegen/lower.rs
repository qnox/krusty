...
impl<'a, 'b, 'c> BodyLowering<'a, 'b, 'c> {
    ...
    fn emit_call(&mut self, func_ref: FuncRef, arguments: &[Value]) -> Result<Inst, Unsupported> {
    ...
    fn unwind_target(&mut self) -> Block {
    ...
    /// agree to.
    fn check_pending(&mut self) -> Result<(), Unsupported> {
    ...
    fn throw(&mut self, operand: u32) -> Result<Option<Value>, Unsupported> {
        ...
        // `kt_throw` records the exception and returns, so this is a store followed by a jump to
        // wherever an exception in flight goes from here — the innermost enclosing `try`, or out
        // of the frame. Deliberately NOT through `Self::runtime_call`: the check that helper emits
        // would branch on a slot this very call just set, which is the jump below written twice.
    ...
    fn lowered_expression(&mut self, id: u32) -> Result<Option<Value>, Unsupported> {
        match self.file.ir.expr(id).clone() {
            ...
            // A `lateinit` read whose storage is not an instance field — a local slot or a
            // top-level property — where common lowering names the guard as its own node instead
            // of leaving it to be inferred from the field. The guard itself is the field read's,
            // because it is the same guard: null is the evidence either way.
