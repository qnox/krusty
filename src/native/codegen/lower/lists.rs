...
fn walk_symbol(
    ...
    Some(match (name, arity) {
        ...
        // `xs.toTypedArray()`: an `Array<T>` holding what the receiver walks. It is a walk and not
        // a list member, because a `Collection` is what it is declared on and a set is one too. A
        // collection's elements are already references, so this copies rather than converts each.
