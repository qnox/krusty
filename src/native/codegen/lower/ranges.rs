...
/// The bounds a counted loop walks and how it walks between them: `a..b`, `a until b`,
/// `a downTo b`.
pub(super) struct CountedRange {
...
impl BodyLowering<'_, '_, '_> {
    ...
    pub(super) fn indices(&mut self, receiver: u32) -> Result<Option<Value>, Unsupported> {
            ...
            .is_some_and(super::super::super::intrinsics::is_list_type)
        {
            ...
            .obj_internal()
            .is_some_and(super::super::super::intrinsics::is_collection_type)
            && !self.file.implements_collection_of(ty)
        {
            // A collection that may be a SET has no list header to read. Its size is how many
            // elements it walks, which for every collection the runtime builds is the same number.
            let value = self.reference(receiver)?;
            if self.terminated {
                return Ok(None);
            }
