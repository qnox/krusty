...
// Read by the CODE GENERATOR, which lands next: an emitted call to a runtime function has to be
// told which names the runtime actually defines, so that one it does not name declines at compile
// time rather than at the link. Nothing in the linker itself asks.
#[allow(dead_code)]
