...
mod fq_tests {
    ...
    #[test]
    fn meta_param_ty_maps_primitives_functions_and_erasure() {
        // A named primitive maps to its `Ty`.
        assert_eq!(meta_param_ty(Some("kotlin/Int")), Ty::Int);
        assert_eq!(meta_param_ty(Some("kotlin/String")), Ty::String);
        // A user class stays an object type.
        assert_eq!(meta_param_ty(Some("demo/Foo")), Ty::obj("demo/Foo"));
        // A function type keeps its arity.
        assert_eq!(
            meta_param_ty(Some("kotlin/Function2")),
            Ty::fun(vec![Ty::obj("kotlin/Any"); 2], Ty::obj("kotlin/Any"))
        );
        assert_eq!(
            meta_param_ty(Some("kotlin/Function0")),
            Ty::fun(vec![], Ty::obj("kotlin/Any"))
        );
        // `kotlin/Function` with no digit suffix is an ordinary object type, not a function.
        assert_eq!(
            meta_param_ty(Some("kotlin/Function")),
            Ty::obj("kotlin/Function")
        );
        // A type-parameter param (no name) erases to `kotlin/Any`.
        assert_eq!(meta_param_ty(None), Ty::obj("kotlin/Any"));
    }

    #[test]
    fn ty_compat_matches_exact_erased_function_and_vararg() {
        // Exact match.
        assert!(ty_compat(&Ty::Int, &Ty::Int));
        assert!(!ty_compat(&Ty::Int, &Ty::Long));
        // An erased `Any`/`Object` on either side matches any reference type.
        assert!(ty_compat(&Ty::obj("kotlin/Any"), &Ty::String));
        assert!(ty_compat(
            &Ty::obj("java/lang/Object"),
            &Ty::obj("demo/Foo")
        ));
        assert!(ty_compat(&Ty::String, &Ty::obj("kotlin/Any")));
        // Distinct class names do not match.
        assert!(!ty_compat(&Ty::obj("demo/A"), &Ty::obj("demo/B")));
        // Erased-Any does NOT match a primitive (not a reference).
        assert!(!ty_compat(&Ty::obj("kotlin/Any"), &Ty::Int));
        // Function params match on arity.
        assert!(ty_compat(
            &Ty::fun(vec![Ty::obj("kotlin/Any")], Ty::obj("kotlin/Any")),
            &Ty::fun(vec![Ty::obj("kotlin/Any")], Ty::Int)
        ));
        // A `@Metadata` vararg (Kotlin array class) aligns with a JVM array descriptor param.
        assert!(ty_compat(&Ty::obj("kotlin/Array"), &Ty::array(Ty::String)));
        assert!(ty_compat(&Ty::obj("kotlin/IntArray"), &Ty::array(Ty::Int)));
    }

    #[test]
    fn is_kotlin_array_class_recognises_the_nine_builtins() {
        for name in [
            "kotlin/Array",
            "kotlin/IntArray",
            "kotlin/LongArray",
            ...
            assert!(
                is_kotlin_array_class(name),
                "{name} should be an array class"
            );
        }
        assert!(!is_kotlin_array_class("kotlin/String"));
        assert!(!is_kotlin_array_class("java/util/List"));
        assert!(!is_kotlin_array_class(""));
    }

    #[test]
    fn compat_prefix_requires_equal_length_and_all_compatible() {
        let meta = [Ty::Int, Ty::obj("kotlin/Any")];
        let desc = [Ty::Int, Ty::String];
        assert!(compat_prefix(&meta, &desc));
        // Length mismatch fails.
        assert!(!compat_prefix(&meta, &[Ty::Int]));
        // One incompatible element fails.
        assert!(!compat_prefix(&[Ty::Int, Ty::Long], &[Ty::Int, Ty::String]));
        // Both empty: trivially compatible.
        assert!(compat_prefix(&[], &[]));
    }

    #[test]
    fn first_descriptor_param_reads_the_leading_type() {
        assert_eq!(
            first_descriptor_param("(ILjava/lang/String;)V"),
            Some("I".to_string())
        );
        assert_eq!(
            first_descriptor_param("(Ljava/lang/String;I)V"),
            Some("Ljava/lang/String;".to_string())
        );
        // An array param is read whole.
        assert_eq!(first_descriptor_param("([IJ)V"), Some("[I".to_string()));
        // No parameters → None.
        assert_eq!(first_descriptor_param("()V"), None);
        // Not a descriptor (no opening paren) → None.
        assert_eq!(first_descriptor_param("I"), None);
    }

    #[test]
    fn descriptor_ret_extracts_the_return_type() {
        assert_eq!(descriptor_ret("(I)V"), Some("V".to_string()));
        assert_eq!(
            descriptor_ret("(II)Ljava/lang/String;"),
            Some("Ljava/lang/String;".to_string())
        );
        assert_eq!(descriptor_ret("()[I"), Some("[I".to_string()));
        // No close paren → None.
        assert_eq!(descriptor_ret("garbage"), None);
    }

    #[test]
    fn read_one_type_advances_past_one_descriptor() {
        let mut s = "IJ";
        assert_eq!(read_one_type(&mut s), "I");
        assert_eq!(s, "J");

        let mut s = "Ljava/lang/String;I";
        assert_eq!(read_one_type(&mut s), "Ljava/lang/String;");
        assert_eq!(s, "I");

        // A nested array consumes its whole element.
        let mut s = "[[Ljava/lang/Object;";
        assert_eq!(read_one_type(&mut s), "[[Ljava/lang/Object;");
        assert_eq!(s, "");

        // An empty input yields the empty type.
        let mut s = "";
        assert_eq!(read_one_type(&mut s), "");
    }

    #[test]
    fn class_internal_from_entry_strips_class_suffix() {
        assert_eq!(
            class_internal_from_entry("java/lang/String.class"),
            Some("java/lang/String")
        );
        // Not a class file.
        assert_eq!(class_internal_from_entry("META-INF/MANIFEST.MF"), None);
        // A bare `.class` with an empty stem is rejected.
        assert_eq!(class_internal_from_entry(".class"), None);
    }

    #[test]
    fn is_type_aliases_kt_matches_file_facades() {
        assert!(is_type_aliases_kt("kotlin/TypeAliasesKt"));
        assert!(is_type_aliases_kt("com/example/LibKt"));
        assert!(is_type_aliases_kt("PlainKt"));
        // A regular class is not a file facade.
        assert!(!is_type_aliases_kt("com/example/Widget"));
        assert!(!is_type_aliases_kt("kotlin/String"));
    }

    #[test]
    fn register_class_name_skips_synthetic_and_registers_bare_names() {
        let mut idx = TypeIndex::default();
        let mut amb = std::collections::HashSet::new();

        // Empty internal is ignored.
        register_class_name("", &mut idx, &mut amb);
        assert!(idx.class_names.is_empty());

        // A nested/synthetic (`$`) name is skipped entirely.
        register_class_name("com/example/Outer$Inner", &mut idx, &mut amb);
        assert!(!idx.class_names.contains_key("Outer$Inner"));
        assert!(!idx.class_names.contains_key("com.example.Outer$Inner"));

        // Module/package descriptors are skipped.
        register_class_name("module-info", &mut idx, &mut amb);
        register_class_name("com/example/package-info", &mut idx, &mut amb);
        assert!(idx.class_names.is_empty());

        // A packageless bare name registers its simple name only (no dotted FQ entry).
        register_class_name("Widget", &mut idx, &mut amb);
        assert_eq!(
            idx.class_names.get("Widget").map(String::as_str),
            Some("Widget")
        );
