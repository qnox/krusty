...
/// The physical JVM method name for a Kotlin mapped-builtin virtual member. Shared by target
/// validation and bytecode emission so a callable reference is checked against the same ABI it calls.
