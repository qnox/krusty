...
mod tests {
    ...
    #[test]
    fn write_verif_type_encodes_every_primitive_tag() {
        let cases = [
            (VerifType::Top, 0u8),
            (VerifType::Integer, 1),
            (VerifType::Float, 2),
        ...
        for (vt, tag) in cases {
            let mut out = Vec::new();
            write_verif_type(&vt, &mut out);
            assert_eq!(out, vec![tag]);
        }
    }

    #[test]
    fn stackmap_two_frames_delta_and_out_of_range_drop() {
        let mut code = CodeBuilder::new(1);
        code.iload(0); // 0
        let l1 = code.new_label();
        ...
        code.iload(0); // 4
        code.ifeq(l2); // 5..8
        code.bind(l1); // offset 8
        code.iload(0); // 8
        code.ret_void(); // 9
        code.bind(l2); // offset 10
        code.ret_void(); // 10 — keeps l2 in range
                         // A label bound one-past-the-end must be dropped by build_stackmap.
        let past = code.new_label();
        code.bind_at(past, code.bytes.len()); // == code_len, out of range
        code.add_frame_if_new(l1, vec![VerifType::Integer], vec![]);
        code.add_frame_if_new(l2, vec![VerifType::Integer], vec![]);
        code.add_frame_if_new(past, vec![VerifType::Integer], vec![]);
        let sm = code.build_stackmap().unwrap();
        // Only the two in-range frames survive.
        assert_eq!(u16::from_be_bytes([sm[0], sm[1]]), 2);
        // First: full_frame, delta = 8.
        assert_eq!(sm[2], 255);
        assert_eq!(u16::from_be_bytes([sm[3], sm[4]]), 8);
        // locals(1) + Integer(1 byte) + stack(0) = header 2 + 1 + 2 = bytes [5..10).
        // Second frame full_frame tag at offset 2 + (3 + 1 + 2) = 8.
        assert_eq!(sm[10], 255);
        // Second delta = 10 - 8 - 1 = 1.
        assert_eq!(u16::from_be_bytes([sm[11], sm[12]]), 1);
    }

    #[test]
    fn resolved_frames_drops_unbound_labels() {
        let mut code = CodeBuilder::new(1);
        code.ret_void(); // offset 0
        let bound = code.new_label();
        code.bind(bound); // offset 1
        let unbound = code.new_label(); // never bound → offset usize::MAX
        code.add_frame_if_new(bound, vec![VerifType::Integer], vec![]);
        code.add_frame_if_new(unbound, vec![VerifType::Float], vec![]);
        let resolved = code.resolved_frames();
        assert_eq!(resolved.len(), 1);
        assert_eq!(resolved[0].0, 1); // the bound offset
        assert_eq!(resolved[0].1.len(), 1);
        assert!(resolved[0].1[0] == VerifType::Integer);
    }

    #[test]
    fn slot_count_counts_wide_constants() {
        let mut cp = ConstPool::default();
        assert_eq!(cp.slot_count(), 0);
        cp.utf8("a"); // 1 slot
        cp.long(1); // 2 slots
        cp.double(2.0); // 2 slots
        cp.integer(3); // 1 slot
        assert_eq!(cp.slot_count(), 6);
    }
    ...
    // ---- single-byte opcode emitters: exact byte + stack tracking -------------------------------
    ...
    fn int_arithmetic_opcodes() {
        ...
        c.irem();
        c.ineg();
        assert_eq!(c.bytes, vec![0x64, 0x68, 0x6c, 0x70, 0x74]);
    }

    #[test]
    fn long_arithmetic_opcodes() {
        let mut c = cb();
        c.ladd();
        c.lsub();
        c.lmul();
        ...
        c.lrem();
        c.lneg();
        assert_eq!(c.bytes, vec![0x61, 0x65, 0x69, 0x6d, 0x71, 0x75]);
    }

    #[test]
    fn double_arithmetic_opcodes() {
        let mut c = cb();
        c.dadd();
        c.dsub();
        c.dmul();
        ...
        c.drem();
        c.dneg();
        assert_eq!(c.bytes, vec![0x63, 0x67, 0x6b, 0x6f, 0x73, 0x77]);
    }

    #[test]
    fn float_arithmetic_and_compare_opcodes() {
        let mut c = cb();
        c.fadd();
        c.fsub();
        c.fmul();
        ...
        assert_eq!(
            c.bytes,
            vec![0x62, 0x66, 0x6a, 0x6e, 0x72, 0x76, 0x96, 0x95]
        );
    }
    ...
    fn int_bitwise_opcodes() {
        ...
        c.ishr();
        c.iushr();
        assert_eq!(c.bytes, vec![0x82, 0x7e, 0x80, 0x78, 0x7a, 0x7c]);
    }

    #[test]
    fn long_bitwise_opcodes() {
        let mut c = cb();
        c.land();
        c.lor();
        c.lxor();
        ...
        c.lshr();
        c.lushr();
        assert_eq!(c.bytes, vec![0x7f, 0x81, 0x83, 0x79, 0x7b, 0x7d]);
    }

    #[test]
    fn numeric_conversion_opcodes() {
        let mut c = cb();
        c.i2l();
        c.i2d();
        c.l2d();
        ...
        assert_eq!(
            c.bytes,
            vec![
                0x85, 0x87, 0x8a, 0x86, 0x89, 0x8d, 0x88, 0x8b, 0x8c, 0x8e, 0x8f, 0x90, 0x91, 0x92,
                0x93
            ]
        );
    }
    ...
    #[test]
    fn typed_return_opcodes() {
        let mut c = cb();
        c.lreturn();
        c.freturn();
        c.dreturn();
        c.areturn();
        assert_eq!(c.bytes, vec![0xad, 0xae, 0xaf, 0xb0]);
    }

    #[test]
    fn stack_manipulation_opcodes_and_tracking() {
        let mut c = cb();
        c.dup(); // +1
        c.dup(); // +1
        assert_eq!(c.max_stack, 2);
        c.pop(); // -1
        c.pop2(); // -2 (clamped at 0)
        assert_eq!(c.bytes, vec![0x59, 0x59, 0x57, 0x58]);
    }

    #[test]
    fn zero_const_opcodes_and_stack_growth() {
        let mut c = cb();
        c.aconst_null(); // +1
        c.lconst_0(); // +2
        c.fconst_0(); // +1
        c.dconst_0(); // +2
        assert_eq!(c.bytes, vec![0x01, 0x09, 0x0b, 0x0e]);
        assert_eq!(c.max_stack, 6);
    }

    #[test]
    fn remaining_compact_loads_and_stores() {
        let mut c = cb();
        c.lload(0); // lload_0 = 0x1e
        c.fload(0); // fload_0 = 0x22
        c.dload(0); // dload_0 = 0x26
        c.aload(0); // aload_0 = 0x2a
        assert_eq!(c.bytes, vec![0x1e, 0x22, 0x26, 0x2a]);
        let mut s = cb();
        s.lstore(0); // lstore_0 = 0x3f
        s.fstore(0); // fstore_0 = 0x43
        s.dstore(0); // dstore_0 = 0x47
        assert_eq!(s.bytes, vec![0x3f, 0x43, 0x47]);
    }

    #[test]
    fn object_and_array_opcodes() {
        let mut c = cb();
        c.athrow(); // 0xbf
        c.arraylength(); // 0xbe
        c.newarray(10); // 0xbc 0x0a (T_INT)
        c.anewarray(5); // 0xbd 0x0005
        c.new_obj(7); // 0xbb 0x0007
        c.instance_of(9); // 0xc1 0x0009
        c.checkcast(9); // 0xc0 0x0009
        assert_eq!(
            c.bytes,
            vec![
                0xbf, 0xbe, 0xbc, 0x0a, 0xbd, 0x00, 0x05, 0xbb, 0x00, 0x07, 0xc1, 0x00, 0x09, 0xc0,
                0x00, 0x09
            ]
        );
    }
    ...
    #[test]
    fn member_dispatch_opcodes_encode_index() {
        let mut c = cb();
        c.invokestatic(5, 0, 1); // 0xb8 0x0005
        c.invokevirtual(6, 1, 0); // 0xb6 0x0006
        c.invokespecial(7, 0, 0); // 0xb7 0x0007
        c.getstatic(8, 1); // 0xb2 0x0008
        c.putstatic(9, 1); // 0xb3 0x0009
        assert_eq!(
            c.bytes,
            vec![
                0xb8, 0x00, 0x05, 0xb6, 0x00, 0x06, 0xb7, 0x00, 0x07, 0xb2, 0x00, 0x08, 0xb3, 0x00,
                0x09
            ]
        );
    }
    ...
    #[test]
    fn field_access_opcodes_encode_index() {
        let mut c = cb();
        c.aconst_null(); // provide a receiver for getfield/putfield accounting
        c.getfield(4, 1); // 0xb4 0x0004
        c.aconst_null();
        c.aconst_null();
        c.putfield(4, 1); // 0xb5 0x0004
        assert_eq!(
            c.bytes,
            vec![0x01, 0xb4, 0x00, 0x04, 0x01, 0x01, 0xb5, 0x00, 0x04]
        );
    }

    #[test]
    fn array_load_store_use_given_opcode() {
        let mut c = cb();
        c.array_load(0x2e, 1); // iaload
        c.array_store(0x4f, 1); // iastore
        assert_eq!(c.bytes, vec![0x2e, 0x4f]);
    }

    #[test]
    fn ldc_class_and_wide_constant_pushes() {
        let mut cw = ClassWriter::new("FooKt", "java/lang/Object");
        let mut c = cb();
        c.push_float(1.5, &mut cw); // ldc2? no: float uses ldc/ldc_w
        c.push_double(2.5, &mut cw); // ldc2_w = 0x14
        c.push_string("hi", &mut cw);
        c.ldc_class("java/lang/String", &mut cw);
        // First byte of push_double is ldc2_w (0x14); push_string/ldc_class emit ldc or ldc_w.
        assert!(c.bytes.contains(&0x14));
