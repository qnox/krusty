//! kotlinc's `RedundantGotoMethodTransformer`, over a [`MethodNode`].
//!
//! It walks the instruction list backwards. Labels and `nop`s are transparent; a line number
...
//! finally goes.
//!
//! The `nop`s a removed `goto` leaves are for the cleanup after it (see `redundant_nops`).
//!
...
mod tests {
    ...
    /// The body's instructions after the pass, or `None` when it changed nothing.
    fn run(body: LabelledBody) -> Option<Vec<Insn>> {
        run_pinned(body, &[])
    }

    fn run_pinned(mut body: LabelledBody, pinned: &[usize]) -> Option<Vec<Insn>> {
        let pinned = pinned.iter().map(|&k| body.labels[k]).collect();
        remove(&mut body.method, &pinned).then(|| body.method.instructions().cloned().collect())
    }

    #[test]
    fn a_goto_to_the_next_instruction_becomes_a_nop() {
        // `s ?: "d"` after the null-check fold: 0 aload_0; 1 dup; 2 ifnonnull 6; 3 pop;
        // 4 aconst_null; 5 goto 6; 6 areturn. The `nop` is the cleanup's to remove.
        ...
        expected.extend(ops(&[0x57, 0x01, NOP, ARETURN]));
    ...
    #[test]
    fn a_goto_alone_on_its_line_becomes_a_nop() {
        // 0 iconst_0; 1 istore_0; 2 (line) goto 3; 3 (line) iload_0; 4 ireturn.
        let body = labelled_body(
            &[Ok(0x03), Ok(0x3b), Err((GOTO, 3)), Ok(0x1a), Ok(IRETURN)],
            &[0, 2, 3],
            None,
        );
        assert_eq!(run(body), Some(ops(&[0x03, 0x3b, NOP, 0x1a, IRETURN])));
    }
    ...
    fn a_goto_to_a_pinned_label_is_neither_threaded_through_nor_removed() {
        ...
        assert_eq!(run_pinned(body, &[6]), None);
    }
    ...
    fn a_long_goto_chain_is_resolved_once_and_removed() {
        ...
        let body = labelled_body(&insns, &[], None);
        let mut expected = vec![body.jump(IFEQ, end)];
        expected.extend(std::iter::repeat_n(Insn::Op(NOP), links));
        expected.push(Insn::Op(ARETURN));
        assert_eq!(run(body), Some(expected));
    }
}
