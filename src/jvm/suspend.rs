...
mod tests {
    ...
    #[test]
    fn helper_type_constructors() {
        assert_eq!(object_ty(), Ty::nullable(Ty::obj("kotlin/Any")));
        assert_eq!(int_ty(), Ty::obj("kotlin/Int"));
        assert_eq!(continuation_ty(), Ty::obj("kotlin/coroutines/Continuation"));
    }

    #[test]
    fn cps_descriptor_appends_continuation_and_erases_return() {
        assert_eq!(
            cps_descriptor("()I"),
            "(Lkotlin/coroutines/Continuation;)Ljava/lang/Object;"
        );
        ...
        assert_eq!(
            cps_descriptor("(Ljava/lang/String;)V"),
            "(Ljava/lang/String;Lkotlin/coroutines/Continuation;)Ljava/lang/Object;"
        );
    }

    #[test]
    fn is_boxed_primitive_internal_covers_kotlin_and_java_wrappers() {
        for n in [
            "kotlin/Int",
            "kotlin/Long",
            "kotlin/Short",
            ...
            assert!(is_boxed_primitive_internal(n), "{n}");
        }
        assert!(!is_boxed_primitive_internal("kotlin/String"));
        assert!(!is_boxed_primitive_internal("java/lang/Object"));
        assert!(!is_boxed_primitive_internal("demo/Foo"));
    }

    #[test]
    fn reference_needs_checkcast_by_type() {
        assert!(reference_needs_checkcast(&Ty::String));
        assert!(reference_needs_checkcast(&Ty::array(Ty::Int)));
        assert!(reference_needs_checkcast(&Ty::obj("demo/Foo")));
        assert!(reference_needs_checkcast(&Ty::nullable(Ty::String)));
        // Any and boxed primitives don't need a checkcast off Object.
        assert!(!reference_needs_checkcast(&Ty::obj("kotlin/Any")));
        assert!(!reference_needs_checkcast(&Ty::obj("kotlin/Int")));
        // A non-reference primitive.
        assert!(!reference_needs_checkcast(&Ty::Int));
        // A type parameter follows its bound.
        assert!(reference_needs_checkcast(&Ty::TyParam("T", &Ty::String)));
        // A type parameter bound by a non-reference primitive follows through to `false`.
        assert!(!reference_needs_checkcast(&Ty::TyParam("T", &Ty::Int)));
    }

    #[test]
    fn spill_field_ty_promotes_unit_to_object() {
        assert_eq!(spill_field_ty(Ty::Unit), Ty::obj("kotlin/Unit"));
        assert_eq!(spill_field_ty(Ty::Int), Ty::Int);
        assert_eq!(spill_field_ty(Ty::obj("demo/Foo")), Ty::obj("demo/Foo"));
    }

    #[test]
    fn suspend_call_detection_local_and_cross_unit() {
        let mut ir = IrFile::default();
        // A direct local call to fid 7.
        let call = ir.add_expr(IrExpr::Call {
            callee: Callee::Local(7),
            dispatch_receiver: None,
            args: vec![],
        });
        // A local call to a non-suspend fid 3.
        let other = ir.add_expr(IrExpr::Call {
            callee: Callee::Local(3),
            dispatch_receiver: None,
            args: vec![],
        });
        let mut set = HashSet::new();
        set.insert(7u32);
        assert_eq!(suspend_call_fid(&ir, call, &set), Some(7));
        assert_eq!(suspend_call_fid(&ir, other, &set), None);
        assert!(is_suspend_call(&ir, call, &set));
        assert!(!is_suspend_call(&ir, other, &set));

        // Cross-unit: recorded in `suspend_calls` even though its fid isn't in the set.
        ir.suspend_calls.insert(other, Ty::Int);
        assert!(is_suspend_call(&ir, other, &set));
    }

    #[test]
    fn expr_calls_suspend_recurses_into_children() {
        let mut ir = IrFile::default();
        let inner = ir.add_expr(IrExpr::Call {
            callee: Callee::Local(7),
            dispatch_receiver: None,
            args: vec![],
        });
        // Wrap the suspend call as a block value.
        ...
        let mut set = HashSet::new();
        set.insert(7u32);
        assert!(expr_calls_suspend(&ir, block, &set));
        // An empty block calls nothing.
        let empty = ir.add_expr(IrExpr::Block {
            stmts: vec![],
            value: None,
        });
        assert!(!expr_calls_suspend(&ir, empty, &set));
    }

    #[test]
    fn find_local_ty_and_max_value_index() {
        let mut ir = IrFile::default();
        let v0 = ir.add_expr(IrExpr::Variable {
            index: 2,
            ty: Ty::Int,
            init: None,
        });
        let v1 = ir.add_expr(IrExpr::Variable {
            index: 5,
            ty: Ty::String,
            init: None,
        });
        ...
        assert_eq!(find_local_ty(&ir, block, 2), Some(Ty::Int));
        assert_eq!(find_local_ty(&ir, block, 5), Some(Ty::String));
        assert_eq!(find_local_ty(&ir, block, 9), None);
        // max_value_index scans the whole arena (highest Variable index = 5).
        assert_eq!(max_value_index(&ir), 5);
        let g = ir.add_expr(IrExpr::GetValue(11));
        let _ = g;
        assert_eq!(max_value_index(&ir), 11);
    }
}
