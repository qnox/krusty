...
//! # Monotonic mode
//!
//! Releases are recorded where kotlinc makes them, but the cursor they would lower is kept: the
//! frame still hands out the slots the emitter always did, so class files are unchanged while every
//! allocation and release goes through this type. A release that kotlinc's stack would reject — a
//! local left while something entered after it is still live — is reported under the `slots` trace
//! category instead of failing, because it names an order the emitter has to change before the
//! cursor may follow the releases.
//!
//! Three cursor movements kotlinc's frame does not make are kept as named operations until the
...
//! entered above it) and [`FrameMap::reserve_through`] (a spliced inline body's locals are claimed
...
impl FrameMap {
    ...
    /// the top one; here that is reported, and the cursor is kept either way (monotonic mode).
    fn leave_at(&mut self, index: usize) {
        let entry = self.entries.remove(index);
        if let Some(above) = self.entries.get(index) {
...
mod tests {
    ...
    #[test]
    fn releases_keep_the_cursor_in_monotonic_mode() {
        let mut frame = FrameMap::default();
