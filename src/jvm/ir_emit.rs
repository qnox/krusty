...
mod signature_tests {
    ...
    fn sig(
        ...
        param_tparams: Vec<Option<String>>,
        ret_tparam: Option<String>,
        supers: Vec<Ty>,
    ) -> IrGenericSig {
        IrGenericSig {
            type_params,
    ...
    // --- ir_ty_to_jvm: the single Kotlin-Ty → erased-JVM-Ty mapping ---

    #[test]
    fn ir_ty_to_jvm_maps_kotlin_primitives_to_jvm_scalars() {
        assert_eq!(ir_ty_to_jvm(&Ty::obj("kotlin/Int")), Ty::Int);
        assert_eq!(ir_ty_to_jvm(&Ty::obj("kotlin/Long")), Ty::Long);
        assert_eq!(ir_ty_to_jvm(&Ty::obj("kotlin/Boolean")), Ty::Boolean);
        assert_eq!(ir_ty_to_jvm(&Ty::obj("kotlin/Char")), Ty::Char);
        assert_eq!(ir_ty_to_jvm(&Ty::obj("kotlin/Double")), Ty::Double);
        assert_eq!(ir_ty_to_jvm(&Ty::obj("kotlin/String")), Ty::String);
    }

    #[test]
    fn ir_ty_to_jvm_erases_nullability_on_reference_and_primitive() {
        // A nullable primitive erases to the same scalar (boxing handled elsewhere).
        assert_eq!(ir_ty_to_jvm(&Ty::nullable(Ty::obj("kotlin/Int"))), Ty::Int);
        // A nullable reference keeps its class identity.
        assert_eq!(
            ir_ty_to_jvm(&Ty::nullable(Ty::obj("demo/Foo"))),
            Ty::obj("demo/Foo")
        );
    }

    #[test]
    fn ir_ty_to_jvm_specializes_primitive_arrays_and_boxes_object_arrays() {
        assert_eq!(
            ir_ty_to_jvm(&Ty::obj("kotlin/IntArray")),
            Ty::array(Ty::Int)
        );
        assert_eq!(
            ir_ty_to_jvm(&Ty::obj("kotlin/CharArray")),
            Ty::array(Ty::Char)
        );
        // Array<Int> boxes its element: `[Ljava/lang/Integer;` — carried as Array(Obj("kotlin/Int")).
        assert_eq!(
            ir_ty_to_jvm(&Ty::obj_args("kotlin/Array", &[Ty::obj("kotlin/Int")])),
            Ty::array(Ty::obj("kotlin/Int"))
        );
        // Array with no element defaults to Object.
        assert_eq!(
            ir_ty_to_jvm(&Ty::obj("kotlin/Array")),
            Ty::array(Ty::obj("java/lang/Object"))
        );
    }

    #[test]
    fn ir_ty_to_jvm_maps_function_type_to_functionn_and_suspend_adds_one() {
        assert_eq!(
            ir_ty_to_jvm(&Ty::fun(vec![Ty::Int], Ty::Unit)),
            Ty::obj("kotlin/jvm/functions/Function1")
        );
        assert_eq!(
            ir_ty_to_jvm(&Ty::fun_suspend(vec![Ty::Int], Ty::Unit)),
            Ty::obj("kotlin/jvm/functions/Function2")
        );
    }

    #[test]
    fn ir_ty_to_jvm_collapses_type_parameter_to_its_bound() {
        assert_eq!(
            ir_ty_to_jvm(&Ty::ty_param("T", Ty::obj("kotlin/Any"))),
            Ty::obj("kotlin/Any")
        );
        assert_eq!(
            ir_ty_to_jvm(&Ty::ty_param("N", Ty::obj("kotlin/Int"))),
            Ty::Int
        );
    }

    #[test]
    fn ir_ty_to_jvm_passes_user_classes_and_unit_through() {
        assert_eq!(ir_ty_to_jvm(&Ty::obj("demo/Foo")), Ty::obj("demo/Foo"));
        assert_eq!(ir_ty_to_jvm(&Ty::Unit), Ty::Unit);
    }

    // --- jvm_type_params: the `<T:bound…>` declaration section ---

    #[test]
    fn jvm_type_params_empty_is_the_empty_string() {
        let g = sig(vec![], vec![], None, vec![]);
        assert_eq!(jvm_type_params(&g), Some(String::new()));
    }

    #[test]
    fn jvm_type_params_single_any_bound_erases_to_object() {
        let g = sig(
            vec![("T".into(), Ty::obj("kotlin/Any"))],
            vec![],
            None,
            vec![],
        );
        assert_eq!(
            jvm_type_params(&g).as_deref(),
            Some("<T:Ljava/lang/Object;>")
        );
    }

    #[test]
    fn jvm_type_params_primitive_bound_boxes_and_reference_bound_erases() {
        let prim = sig(
            vec![("N".into(), Ty::obj("kotlin/Int"))],
            vec![],
            None,
            vec![],
        );
        assert_eq!(
            jvm_type_params(&prim).as_deref(),
            Some("<N:Ljava/lang/Integer;>")
        );
        let reference = sig(
            vec![("T".into(), Ty::obj("demo/Foo"))],
            vec![],
            None,
            vec![],
        );
        assert_eq!(
            jvm_type_params(&reference).as_deref(),
            Some("<T:Ldemo/Foo;>")
        );
    }

    #[test]
    fn jvm_type_params_multiple_params_concatenate_in_order() {
        let g = sig(
            vec![
                ("T".into(), Ty::obj("kotlin/Any")),
                ("N".into(), Ty::obj("kotlin/Int")),
            ],
            vec![],
            None,
            vec![],
        ...
        assert_eq!(
            jvm_type_params(&g).as_deref(),
            Some("<T:Ljava/lang/Object;N:Ljava/lang/Integer;>")
        );
    }

    #[test]
    fn jvm_type_params_unrepresentable_bound_is_none() {
        // `Ty::Null` has no JVM erasure (ir_ty_to_jvm → Error), so a bound of it can't be represented.
        let g = sig(vec![("T".into(), Ty::Null)], vec![], None, vec![]);
        assert_eq!(jvm_type_params(&g), None);
    }

    // --- jvm_bound_descriptor ---

    #[test]
    fn jvm_bound_descriptor_covers_any_primitive_string_and_reference() {
        assert_eq!(
            jvm_bound_descriptor(&Ty::obj("kotlin/Any")).as_deref(),
            Some("Ljava/lang/Object;")
        );
        assert_eq!(
            jvm_bound_descriptor(&Ty::obj("kotlin/Int")).as_deref(),
            Some("Ljava/lang/Integer;")
        );
        assert_eq!(
            jvm_bound_descriptor(&Ty::obj("kotlin/String")).as_deref(),
            Some("Ljava/lang/String;")
        );
        assert_eq!(
            jvm_bound_descriptor(&Ty::obj("demo/Foo")).as_deref(),
            Some("Ldemo/Foo;")
        );
        assert_eq!(jvm_bound_descriptor(&Ty::Null), None);
    }

    // --- jvm_method_signature ---

    #[test]
    fn jvm_method_signature_identity_uses_type_variable_positions() {
        // fun <T> f(x: T): T  →  <T:Ljava/lang/Object;>(TT;)TT;
        let g = sig(
            vec![("T".into(), Ty::obj("kotlin/Any"))],
            vec![Some("T".into())],
            Some("T".into()),
        ...
        let f = func(vec![Ty::obj("kotlin/Any")], Ty::obj("kotlin/Any"));
        assert_eq!(
            jvm_method_signature(&g, &f).as_deref(),
            Some("<T:Ljava/lang/Object;>(TT;)TT;")
        );
    }

    #[test]
    fn jvm_method_signature_concrete_positions_use_erased_descriptors() {
        // fun <T> f(n: Int): Unit  (declares T but uses concrete param/return) → <...>(I)V
        let g = sig(
            vec![("T".into(), Ty::obj("kotlin/Any"))],
            vec![None],
            None,
        ...
        let f = func(vec![Ty::obj("kotlin/Int")], Ty::Unit);
        assert_eq!(
            jvm_method_signature(&g, &f).as_deref(),
            Some("<T:Ljava/lang/Object;>(I)V")
        );
    }

    #[test]
    fn jvm_method_signature_no_type_params_formats_plain_descriptors() {
        let g = sig(vec![], vec![None], None, vec![]);
        let f = func(vec![Ty::obj("kotlin/String")], Ty::obj("kotlin/Int"));
        assert_eq!(
            jvm_method_signature(&g, &f).as_deref(),
    ...
    #[test]
    fn jvm_method_signature_unrepresentable_bound_propagates_none() {
        let g = sig(vec![("T".into(), Ty::Null)], vec![None], None, vec![]);
        let f = func(vec![Ty::obj("kotlin/Int")], Ty::Unit);
        assert_eq!(jvm_method_signature(&g, &f), None);
    }

    // --- ty_generic_sig: a Ty as a generic-signature element ---

    #[test]
    fn ty_generic_sig_boxes_primitives_and_maps_builtins() {
        assert_eq!(
            ty_generic_sig(&Ty::Int).as_deref(),
            Some("Ljava/lang/Integer;")
        );
        assert_eq!(
            ty_generic_sig(&Ty::String).as_deref(),
            Some("Ljava/lang/String;")
        );
        assert_eq!(ty_generic_sig(&Ty::Unit).as_deref(), Some("Lkotlin/Unit;"));
        assert_eq!(
            ty_generic_sig(&Ty::obj("kotlin/Any")).as_deref(),
            Some("Ljava/lang/Object;")
        );
        assert_eq!(
            ty_generic_sig(&Ty::obj("demo/Foo")).as_deref(),
            Some("Ldemo/Foo;")
        );
    }

    #[test]
    fn ty_generic_sig_recurses_into_type_arguments() {
        // demo/Box<Int> → Ldemo/Box<Ljava/lang/Integer;>;
        let t = Ty::obj_args("demo/Box", &[Ty::Int]);
        assert_eq!(
            ty_generic_sig(&t).as_deref(),
            Some("Ldemo/Box<Ljava/lang/Integer;>;")
        );
    }

    #[test]
    fn ty_generic_sig_unrepresentable_shape_is_none() {
        assert_eq!(ty_generic_sig(&Ty::fun(vec![], Ty::Unit)), None);
    }

    // --- jvm_class_signature ---

    #[test]
    fn jvm_class_signature_plain_generic_extends_object() {
        let g = sig(
            vec![("T".into(), Ty::obj("kotlin/Any"))],
            vec![],
            None,
        ...
        assert_eq!(
            jvm_class_signature(&g).as_deref(),
            Some("<T:Ljava/lang/Object;>Ljava/lang/Object;")
        );
    }

    #[test]
    fn jvm_class_signature_formats_parameterized_supertypes() {
        // No own type params; supertypes Object + demo/Box<Int>.
        let g = sig(
            vec![],
            vec![],
            None,
            vec![Ty::obj("kotlin/Any"), Ty::obj_args("demo/Box", &[Ty::Int])],
        );
        assert_eq!(
            jvm_class_signature(&g).as_deref(),
            Some("Ljava/lang/Object;Ldemo/Box<Ljava/lang/Integer;>;")
        );
    }
}
...
mod pure_helper_tests {
    ...
    #[test]
    fn sam_descriptor_uses_object_params_and_return() {
        assert_eq!(sam_descriptor(0), "()Ljava/lang/Object;");
        assert_eq!(
            sam_descriptor(2),
    ...
    #[test]
    fn boxed_descriptor_boxes_primitives_and_passes_references() {
        assert_eq!(boxed_descriptor(Ty::Int), "Ljava/lang/Integer;");
        assert_eq!(boxed_descriptor(Ty::Boolean), "Ljava/lang/Boolean;");
        // A non-primitive reference keeps its ordinary descriptor.
        assert_eq!(boxed_descriptor(Ty::String), "Ljava/lang/String;");
        assert_eq!(boxed_descriptor(Ty::obj("demo/Foo")), "Ldemo/Foo;");
    }

    // --- ty_from_descriptor_ret / ty_from_field_descriptor ---

    #[test]
    fn ty_from_descriptor_ret_reads_after_close_paren() {
        assert_eq!(ty_from_descriptor_ret("(I)J"), Ty::Long);
        assert_eq!(ty_from_descriptor_ret("()V"), Ty::Unit);
        assert_eq!(
            ty_from_descriptor_ret("(II)Ldemo/Foo;"),
            Ty::obj("demo/Foo")
        );
        // No `)` at all: the whole string is read as a field descriptor ('g' → Error).
        assert_eq!(ty_from_descriptor_ret("garbage"), Ty::Error);
    }

    #[test]
    fn ty_from_field_descriptor_covers_every_prefix() {
        assert_eq!(ty_from_field_descriptor("I"), Ty::Int);
        assert_eq!(ty_from_field_descriptor("J"), Ty::Long);
        assert_eq!(ty_from_field_descriptor("Z"), Ty::Boolean);
        assert_eq!(ty_from_field_descriptor("B"), Ty::Byte);
        assert_eq!(ty_from_field_descriptor("C"), Ty::Char);
        assert_eq!(ty_from_field_descriptor("S"), Ty::Short);
        assert_eq!(ty_from_field_descriptor("F"), Ty::Float);
        assert_eq!(ty_from_field_descriptor("D"), Ty::Double);
        assert_eq!(ty_from_field_descriptor("V"), Ty::Unit);
        assert_eq!(ty_from_field_descriptor("Ldemo/Foo;"), Ty::obj("demo/Foo"));
        assert_eq!(ty_from_field_descriptor("[I"), Ty::array(Ty::Int));
        assert_eq!(
            ty_from_field_descriptor("[[Ldemo/Foo;"),
            Ty::array(Ty::array(Ty::obj("demo/Foo")))
        );
        // Empty / unknown → Error.
        assert_eq!(ty_from_field_descriptor(""), Ty::Error);
        assert_eq!(ty_from_field_descriptor("Q"), Ty::Error);
    }

    // --- ref_internal ---

    #[test]
    fn ref_internal_erases_kotlin_names_and_defaults_to_object() {
        assert_eq!(ref_internal(Ty::String), "java/lang/String");
        assert_eq!(ref_internal(Ty::obj("demo/Foo")), "demo/Foo");
        // Kotlin collection built-in erases to its JVM identity.
        assert_eq!(
            ref_internal(Ty::obj("kotlin/collections/MutableList")),
            "java/util/List"
        );
        // Array becomes a descriptor.
        assert_eq!(ref_internal(Ty::array(Ty::Int)), "[I");
        // A primitive is not a reference → Object.
        assert_eq!(ref_internal(Ty::Int), "java/lang/Object");
    }

    // --- intrinsic_ret ---

    #[test]
    fn intrinsic_ret_maps_named_and_suffixed_intrinsics() {
        assert_eq!(intrinsic_ret("kotlin/String.plus"), Ty::String);
        assert_eq!(intrinsic_ret("kotlin/Any.toString"), Ty::String);
        assert_eq!(intrinsic_ret("kotlin/Any.hashCode"), Ty::Int);
        assert_eq!(intrinsic_ret("kotlin/String.length"), Ty::Int);
        assert_eq!(intrinsic_ret("kotlin/Array.size"), Ty::Int);
        assert_eq!(intrinsic_ret("java/lang/Enum.ordinal"), Ty::Int);
        assert_eq!(intrinsic_ret("kotlin/String.get"), Ty::Char);
        assert_eq!(intrinsic_ret("kotlin/Array.set"), Ty::Unit);
        assert_eq!(intrinsic_ret("java/lang/Enum.name"), Ty::String);
        assert_eq!(intrinsic_ret("demo/Foo.hashCode"), Ty::Int);
        assert_eq!(intrinsic_ret("demo/Foo.compare"), Ty::Int);
        assert_eq!(intrinsic_ret("demo/Foo.other"), Ty::Error);
    }

    // --- prim_array_elem_ty / prim_array_atype ---

    #[test]
    fn prim_array_elem_ty_matches_full_fqname_only() {
        assert_eq!(prim_array_elem_ty("kotlin/IntArray.<init>"), Some(Ty::Int));
        assert_eq!(
            prim_array_elem_ty("kotlin/LongArray.<init>"),
            Some(Ty::Long)
        );
        assert_eq!(
            prim_array_elem_ty("kotlin/DoubleArray.<init>"),
            Some(Ty::Double)
        );
        assert_eq!(
            prim_array_elem_ty("kotlin/FloatArray.<init>"),
            Some(Ty::Float)
        );
        assert_eq!(
            prim_array_elem_ty("kotlin/BooleanArray.<init>"),
            Some(Ty::Boolean)
        );
        assert_eq!(
            prim_array_elem_ty("kotlin/CharArray.<init>"),
            Some(Ty::Char)
        );
        assert_eq!(
            prim_array_elem_ty("kotlin/ByteArray.<init>"),
            Some(Ty::Byte)
        );
        assert_eq!(
            prim_array_elem_ty("kotlin/ShortArray.<init>"),
            Some(Ty::Short)
        );
        // A user class ending in `Array` is not a primitive-array ctor.
        assert_eq!(prim_array_elem_ty("demo/MyArray.<init>"), None);
    }

    #[test]
    fn prim_array_atype_covers_all_newarray_codes() {
        assert_eq!(prim_array_atype("kotlin/BooleanArray.<init>"), 4);
        assert_eq!(prim_array_atype("kotlin/CharArray.<init>"), 5);
        assert_eq!(prim_array_atype("kotlin/FloatArray.<init>"), 6);
        assert_eq!(prim_array_atype("kotlin/DoubleArray.<init>"), 7);
        assert_eq!(prim_array_atype("kotlin/ByteArray.<init>"), 8);
        assert_eq!(prim_array_atype("kotlin/ShortArray.<init>"), 9);
        assert_eq!(prim_array_atype("kotlin/LongArray.<init>"), 11);
        assert_eq!(prim_array_atype("kotlin/IntArray.<init>"), 10);
        // A non-primitive-array fqname falls through to the Int atype (10).
        assert_eq!(prim_array_atype("demo/Foo.<init>"), 10);
    }

    // --- boxed_prim_of ---

    #[test]
    fn boxed_prim_of_unwraps_kotlin_boxed_primitives() {
        assert_eq!(boxed_prim_of(Ty::obj("kotlin/Int")), Some(Ty::Int));
        assert_eq!(boxed_prim_of(Ty::obj("kotlin/Long")), Some(Ty::Long));
        assert_eq!(boxed_prim_of(Ty::obj("kotlin/Short")), Some(Ty::Short));
        assert_eq!(boxed_prim_of(Ty::obj("kotlin/Byte")), Some(Ty::Byte));
        assert_eq!(boxed_prim_of(Ty::obj("kotlin/Double")), Some(Ty::Double));
        assert_eq!(boxed_prim_of(Ty::obj("kotlin/Float")), Some(Ty::Float));
        assert_eq!(boxed_prim_of(Ty::obj("kotlin/Boolean")), Some(Ty::Boolean));
        assert_eq!(boxed_prim_of(Ty::obj("kotlin/Char")), Some(Ty::Char));
        assert_eq!(boxed_prim_of(Ty::obj("kotlin/String")), None);
        assert_eq!(boxed_prim_of(Ty::Int), None);
    }

    // --- array_load_op / array_store_op ---

    #[test]
    fn array_load_op_selects_opcode_and_words() {
        assert_eq!(array_load_op(Ty::Int), (0x2e, 1));
        assert_eq!(array_load_op(Ty::UInt), (0x2e, 1));
        assert_eq!(array_load_op(Ty::Long), (0x2f, 2));
        assert_eq!(array_load_op(Ty::ULong), (0x2f, 2));
        assert_eq!(array_load_op(Ty::Float), (0x30, 1));
        assert_eq!(array_load_op(Ty::Double), (0x31, 2));
        assert_eq!(array_load_op(Ty::Boolean), (0x33, 1));
        assert_eq!(array_load_op(Ty::Byte), (0x33, 1));
        assert_eq!(array_load_op(Ty::Char), (0x34, 1));
        assert_eq!(array_load_op(Ty::Short), (0x35, 1));
        // Reference elements → aaload.
        assert_eq!(array_load_op(Ty::String), (0x32, 1));
    }

    #[test]
    fn array_store_op_selects_opcode_and_words() {
        assert_eq!(array_store_op(Ty::Int), (0x4f, 1));
        assert_eq!(array_store_op(Ty::UInt), (0x4f, 1));
        assert_eq!(array_store_op(Ty::Long), (0x50, 2));
        assert_eq!(array_store_op(Ty::ULong), (0x50, 2));
        assert_eq!(array_store_op(Ty::Float), (0x51, 1));
        assert_eq!(array_store_op(Ty::Double), (0x52, 2));
        assert_eq!(array_store_op(Ty::Boolean), (0x54, 1));
        assert_eq!(array_store_op(Ty::Byte), (0x54, 1));
        assert_eq!(array_store_op(Ty::Char), (0x55, 1));
        assert_eq!(array_store_op(Ty::Short), (0x56, 1));
        assert_eq!(array_store_op(Ty::String), (0x53, 1));
    }

    // --- prim_newarray_atype ---

    #[test]
    fn prim_newarray_atype_covers_all_primitives() {
        assert_eq!(prim_newarray_atype(Ty::Boolean), 4);
        assert_eq!(prim_newarray_atype(Ty::Char), 5);
        assert_eq!(prim_newarray_atype(Ty::Float), 6);
        assert_eq!(prim_newarray_atype(Ty::Double), 7);
        assert_eq!(prim_newarray_atype(Ty::Byte), 8);
        assert_eq!(prim_newarray_atype(Ty::Short), 9);
        assert_eq!(prim_newarray_atype(Ty::Long), 11);
        assert_eq!(prim_newarray_atype(Ty::Int), 10);
    }

    // --- ret_is_nothing / call_ret_ty / norm_nothing ---

    #[test]
    fn norm_nothing_collapses_void_object() {
        assert_eq!(norm_nothing(Ty::obj("java/lang/Void")), Ty::Nothing);
        assert_eq!(norm_nothing(Ty::Int), Ty::Int);
        assert_eq!(norm_nothing(Ty::obj("demo/Foo")), Ty::obj("demo/Foo"));
    }

    #[test]
    fn ret_is_nothing_only_for_non_nullable_bottom() {
        assert!(ret_is_nothing(&Ty::Nothing));
        assert!(ret_is_nothing(&Ty::obj("java/lang/Void")));
        // Nothing? is a real nullable reference, not diverging.
        assert!(!ret_is_nothing(&Ty::nullable(Ty::obj("java/lang/Void"))));
        assert!(!ret_is_nothing(&Ty::obj("kotlin/Int")));
    }

    #[test]
    fn call_ret_ty_maps_nothing_and_erases_others() {
        assert_eq!(call_ret_ty(&Ty::Nothing), Ty::Nothing);
        assert_eq!(call_ret_ty(&Ty::obj("kotlin/Int")), Ty::Int);
        assert_eq!(call_ret_ty(&Ty::obj("demo/Foo")), Ty::obj("demo/Foo"));
    }

    // --- array_jvm_element ---

    #[test]
    fn array_jvm_element_reads_element_or_defaults_object() {
        assert_eq!(array_jvm_element(&Ty::obj("kotlin/IntArray")), Ty::Int);
        assert_eq!(
            array_jvm_element(&Ty::obj_args("kotlin/Array", &[Ty::obj("demo/Foo")])),
            Ty::obj("demo/Foo")
        );
        // A non-array type defaults to Object.
        assert_eq!(
            array_jvm_element(&Ty::obj("demo/Foo")),
    ...
    #[test]
    fn swap_cmp_reverses_order_relations_only() {
        assert_eq!(swap_cmp(IrBinOp::Lt), IrBinOp::Gt);
        assert_eq!(swap_cmp(IrBinOp::Le), IrBinOp::Ge);
        assert_eq!(swap_cmp(IrBinOp::Gt), IrBinOp::Lt);
        assert_eq!(swap_cmp(IrBinOp::Ge), IrBinOp::Le);
        assert_eq!(swap_cmp(IrBinOp::Eq), IrBinOp::Eq);
        assert_eq!(swap_cmp(IrBinOp::Add), IrBinOp::Add);
    }

    // --- valueof_desc ---

    #[test]
    fn valueof_desc_selects_string_valueof_overload() {
        assert_eq!(valueof_desc(Ty::Int), "(I)Ljava/lang/String;");
        assert_eq!(valueof_desc(Ty::Short), "(I)Ljava/lang/String;");
        assert_eq!(valueof_desc(Ty::Byte), "(I)Ljava/lang/String;");
        assert_eq!(valueof_desc(Ty::Long), "(J)Ljava/lang/String;");
        assert_eq!(valueof_desc(Ty::Float), "(F)Ljava/lang/String;");
        assert_eq!(valueof_desc(Ty::Double), "(D)Ljava/lang/String;");
        assert_eq!(valueof_desc(Ty::Boolean), "(Z)Ljava/lang/String;");
        assert_eq!(valueof_desc(Ty::Char), "(C)Ljava/lang/String;");
        assert_eq!(
            valueof_desc(Ty::String),
    ...
    #[test]
    fn ir_ty_nullable_reflects_nullable_wrapper() {
        assert!(ir_ty_nullable(&Ty::nullable(Ty::String)));
        assert!(!ir_ty_nullable(&Ty::String));
    }

    // --- slot_words ---

    #[test]
    fn slot_words_two_for_wide_zero_for_void() {
        assert_eq!(slot_words(Ty::Long), 2);
        assert_eq!(slot_words(Ty::Double), 2);
        assert_eq!(slot_words(Ty::ULong), 2);
        assert_eq!(slot_words(Ty::Unit), 0);
        assert_eq!(slot_words(Ty::Nothing), 0);
        assert_eq!(slot_words(Ty::Int), 1);
        assert_eq!(slot_words(Ty::String), 1);
    }

    // --- mapped_builtin_virtual_name ---

    #[test]
    fn mapped_builtin_virtual_name_rewrites_known_pairs() {
        assert_eq!(
            mapped_builtin_virtual_name("java/lang/CharSequence", "get"),
            "charAt"
        );
        assert_eq!(
            mapped_builtin_virtual_name("kotlin/String", "get"),
            "charAt"
        );
        assert_eq!(
            mapped_builtin_virtual_name("kotlin/ranges/IntRange", "start"),
            "getFirst"
        );
        assert_eq!(
            mapped_builtin_virtual_name("kotlin/ranges/LongRange", "endInclusive"),
            "getLast"
        );
        assert_eq!(
            mapped_builtin_virtual_name("java/util/Map", "keys"),
            "keySet"
        );
        assert_eq!(
            mapped_builtin_virtual_name("kotlin/collections/Map", "entries"),
            "entrySet"
        );
        assert_eq!(
            mapped_builtin_virtual_name("kotlin/reflect/KProperty", "name"),
            "getName"
        );
        assert_eq!(
            mapped_builtin_virtual_name("java/lang/Number", "toInt"),
            "intValue"
        );
        assert_eq!(
            mapped_builtin_virtual_name("java/lang/Number", "toDouble"),
            "doubleValue"
        );
        // Unmapped pair is returned unchanged.
        assert_eq!(mapped_builtin_virtual_name("demo/Foo", "bar"), "bar");
    }

    // --- is_string_plus_virtual ---

    #[test]
    fn is_string_plus_virtual_requires_exact_shape() {
        assert!(is_string_plus_virtual(
            "java/lang/String",
            "plus",
            "(Ljava/lang/Object;)Ljava/lang/String;"
        ));
    ...
    // --- range_to_virtual_ctor ---

    #[test]
    fn range_to_virtual_ctor_covers_int_long_char_and_misses() {
        assert_eq!(
            range_to_virtual_ctor("kotlin/Int", "rangeTo", "(I)Lkotlin/ranges/IntRange;"),
            Some(("kotlin/ranges/IntRange", "(II)V", 2, Ty::Int))
        );
        assert_eq!(
            range_to_virtual_ctor("kotlin/Long", "rangeTo", "(J)Lkotlin/ranges/LongRange;"),
            Some(("kotlin/ranges/LongRange", "(JJ)V", 4, Ty::Long))
        );
        assert_eq!(
            range_to_virtual_ctor("kotlin/Char", "rangeTo", "(C)Lkotlin/ranges/CharRange;"),
            Some(("kotlin/ranges/CharRange", "(CC)V", 2, Ty::Char))
        );
        // Wrong method name.
        assert_eq!(
            range_to_virtual_ctor("kotlin/Int", "until", "(I)Lkotlin/ranges/IntRange;"),
            None
        );
        // Unknown owner/descriptor combo.
        assert_eq!(
            range_to_virtual_ctor("demo/Foo", "rangeTo", "(I)Ldemo/Foo;"),
            None
        );
    }

    // --- wrapper_owner_primitive ---

    #[test]
    fn wrapper_owner_primitive_maps_wrappers_to_primitives() {
        assert_eq!(wrapper_owner_primitive("java/lang/Integer"), Some(Ty::Int));
        assert_eq!(wrapper_owner_primitive("kotlin/Int"), Some(Ty::Int));
        assert_eq!(wrapper_owner_primitive("java/lang/Long"), Some(Ty::Long));
        assert_eq!(wrapper_owner_primitive("kotlin/Double"), Some(Ty::Double));
        assert_eq!(wrapper_owner_primitive("java/lang/Float"), Some(Ty::Float));
        assert_eq!(
            wrapper_owner_primitive("java/lang/Boolean"),
            Some(Ty::Boolean)
        );
        assert_eq!(wrapper_owner_primitive("kotlin/Char"), Some(Ty::Char));
        assert_eq!(wrapper_owner_primitive("kotlin/Byte"), Some(Ty::Byte));
        assert_eq!(wrapper_owner_primitive("java/lang/Short"), Some(Ty::Short));
        assert_eq!(wrapper_owner_primitive("java/lang/String"), None);
    }

    // --- checkcast_internal ---

    #[test]
    fn checkcast_internal_skips_object_and_primitives() {
        assert_eq!(
            checkcast_internal(Ty::String).as_deref(),
            Some("java/lang/String")
        );
        assert_eq!(
            checkcast_internal(Ty::array(Ty::Int)).as_deref(),
            Some("[I")
        );
        assert_eq!(
            checkcast_internal(Ty::obj("demo/Foo")).as_deref(),
            Some("demo/Foo")
        );
        // Object / Any / primitives need no checkcast.
        assert_eq!(checkcast_internal(Ty::obj("java/lang/Object")), None);
        assert_eq!(checkcast_internal(Ty::obj("kotlin/Any")), None);
        assert_eq!(checkcast_internal(Ty::Int), None);
    }

    // --- prim_wrapper ---

    #[test]
    fn prim_wrapper_maps_primitives_to_wrapper_and_descriptor() {
        assert_eq!(prim_wrapper(Ty::Boolean), Some(("java/lang/Boolean", "Z")));
        assert_eq!(prim_wrapper(Ty::Byte), Some(("java/lang/Byte", "B")));
        assert_eq!(prim_wrapper(Ty::Short), Some(("java/lang/Short", "S")));
        assert_eq!(prim_wrapper(Ty::Char), Some(("java/lang/Character", "C")));
        assert_eq!(prim_wrapper(Ty::Int), Some(("java/lang/Integer", "I")));
        assert_eq!(prim_wrapper(Ty::Long), Some(("java/lang/Long", "J")));
        assert_eq!(prim_wrapper(Ty::Float), Some(("java/lang/Float", "F")));
        assert_eq!(prim_wrapper(Ty::Double), Some(("java/lang/Double", "D")));
        assert_eq!(prim_wrapper(Ty::String), None);
    }

    // --- java_string_hash ---

    #[test]
    fn java_string_hash_matches_jdk_string_hashcode() {
        assert_eq!(java_string_hash(""), 0);
        assert_eq!(java_string_hash("a"), 97);
        // "value" per java.lang.String.hashCode.
    ...
    #[test]
    fn arrays_param_desc_specializes_primitive_arrays() {
        assert_eq!(arrays_param_desc(&Ty::Int), "[I");
        assert_eq!(arrays_param_desc(&Ty::Long), "[J");
        assert_eq!(arrays_param_desc(&Ty::Double), "[D");
        assert_eq!(arrays_param_desc(&Ty::Boolean), "[Z");
        // Reference elements use the Object[] overload.
        assert_eq!(arrays_param_desc(&Ty::String), "[Ljava/lang/Object;");
        assert_eq!(
            arrays_param_desc(&Ty::obj("demo/Foo")),
            "[Ljava/lang/Object;"
        );
    }
    ...
    #[test]
    fn ref_class_selects_typed_ref_holder() {
        // ref_class runs its element through ir_ty_to_jvm, so it takes Kotlin-level types.
        assert_eq!(
            ref_class(&Ty::obj("kotlin/Int")),
            ("kotlin/jvm/internal/Ref$IntRef", "I")
        );
        assert_eq!(
            ref_class(&Ty::obj("kotlin/Long")),
            ("kotlin/jvm/internal/Ref$LongRef", "J")
        );
        assert_eq!(
            ref_class(&Ty::obj("kotlin/Boolean")),
            ("kotlin/jvm/internal/Ref$BooleanRef", "Z")
        );
        assert_eq!(
            ref_class(&Ty::obj("kotlin/Char")),
            ("kotlin/jvm/internal/Ref$CharRef", "C")
        );
        assert_eq!(
            ref_class(&Ty::obj("kotlin/Byte")),
            ("kotlin/jvm/internal/Ref$ByteRef", "B")
        );
        assert_eq!(
            ref_class(&Ty::obj("kotlin/Short")),
            ("kotlin/jvm/internal/Ref$ShortRef", "S")
        );
        assert_eq!(
            ref_class(&Ty::obj("kotlin/Float")),
            ("kotlin/jvm/internal/Ref$FloatRef", "F")
        );
        assert_eq!(
            ref_class(&Ty::obj("kotlin/Double")),
            ("kotlin/jvm/internal/Ref$DoubleRef", "D")
        );
        // Reference elements use the object holder.
        assert_eq!(
            ref_class(&Ty::obj("demo/Foo")),
            ("kotlin/jvm/internal/Ref$ObjectRef", "Ljava/lang/Object;")
        );
    }

    // --- parse_descriptor_params ---

    #[test]
    fn parse_descriptor_params_splits_mixed_descriptor() {
        let ps = parse_descriptor_params("(ILjava/lang/String;[IJ)V").expect("parses");
        assert_eq!(ps.len(), 4);
        assert_eq!(ps[0], Ty::Int);
        assert_eq!(ps[1], Ty::String);
        assert_eq!(ps[2], Ty::array(Ty::Int));
        assert_eq!(ps[3], Ty::Long);
        // Empty parameter list.
        assert_eq!(parse_descriptor_params("()V"), Some(vec![]));
        // Missing open paren.
        assert_eq!(parse_descriptor_params("bad"), None);
    }

    // --- vtype_to_verif / expand_collapsed_locals / collapse_locals ---

    #[test]
    fn vtype_to_verif_maps_each_variant() {
        // VerifType has no Debug, so compare via PartialEq with `assert!`.
        use crate::jvm::inline::VType;
        assert!(vtype_to_verif(&VType::Top) == VerifType::Top);
        assert!(vtype_to_verif(&VType::Int) == VerifType::Integer);
        assert!(vtype_to_verif(&VType::Float) == VerifType::Float);
        assert!(vtype_to_verif(&VType::Long) == VerifType::Long);
        assert!(vtype_to_verif(&VType::Double) == VerifType::Double);
        assert!(vtype_to_verif(&VType::Null) == VerifType::Null);
        assert!(vtype_to_verif(&VType::Object(7)) == VerifType::Object(7));
        // Uninitialized forms collapse to Top for the free-standing conversion.
        assert!(vtype_to_verif(&VType::UninitThis) == VerifType::Top);
        assert!(vtype_to_verif(&VType::Uninit(3)) == VerifType::Top);
    }

    #[test]
    fn expand_and_collapse_locals_round_trip_wide_slots() {
        let collapsed = vec![
            VerifType::Integer,
            VerifType::Long,
            VerifType::Object(4),
        ...
        let expanded = expand_collapsed_locals(&collapsed);
        // Long and Double each gain a trailing Top filler slot.
        assert!(
            expanded
                == vec![
                    VerifType::Integer,
    ...
    // --- generic-signature formatting: jvm_type_params / jvm_bound_descriptor / ty_generic_sig ---

    fn gsig(type_params: Vec<(&str, Ty)>, supers: Vec<Ty>) -> crate::ir::IrGenericSig {
    ...
    #[test]
    fn jvm_type_params_empty_when_no_params() {
        let g = gsig(vec![], vec![]);
        assert_eq!(jvm_type_params(&g), Some(String::new()));
    }

    #[test]
    fn jvm_type_params_formats_object_and_primitive_bounds() {
        // Unbounded param → Object; primitive-bounded → boxed wrapper.
        let g = gsig(
            vec![("T", Ty::obj("kotlin/Any")), ("N", Ty::obj("kotlin/Int"))],
            vec![],
        );
        assert_eq!(
            jvm_type_params(&g),
            Some("<T:Ljava/lang/Object;N:Ljava/lang/Integer;>".to_string())
        );
    }

    #[test]
    fn jvm_bound_descriptor_maps_bound_categories() {
        // Any → Object.
        assert_eq!(
            jvm_bound_descriptor(&Ty::obj("kotlin/Any")),
            Some("Ljava/lang/Object;".to_string())
        );
        // Primitive → boxed wrapper.
        assert_eq!(
            jvm_bound_descriptor(&Ty::obj("kotlin/Int")),
            Some("Ljava/lang/Integer;".to_string())
        );
        // String reference bound (as ir-lowered `Obj("kotlin/String")`).
        assert_eq!(
            jvm_bound_descriptor(&Ty::obj("kotlin/String")),
            Some("Ljava/lang/String;".to_string())
        );
        // A user-class reference bound emits its erased class descriptor.
        assert_eq!(
            jvm_bound_descriptor(&Ty::obj("demo/Foo")),
            Some("Ldemo/Foo;".to_string())
        );
    }

    #[test]
    fn ty_generic_sig_boxes_primitives_and_recurses_args() {
        // A scalar in a generic position boxes.
        assert_eq!(
            ty_generic_sig(&Ty::Int),
            Some("Ljava/lang/Integer;".to_string())
        );
        assert_eq!(
            ty_generic_sig(&Ty::String),
            Some("Ljava/lang/String;".to_string())
        );
        assert_eq!(ty_generic_sig(&Ty::Unit), Some("Lkotlin/Unit;".to_string()));
        // A parameterized reference recurses its type arguments (boxing the Int arg). Use a
        // user-defined owner so `to_jvm_internal` leaves the name untouched.
        let box_of_int = Ty::obj_args("demo/Box", &[Ty::Int]);
        assert_eq!(
            ty_generic_sig(&box_of_int),
            Some("Ldemo/Box<Ljava/lang/Integer;>;".to_string())
        );
    }

    #[test]
    fn jvm_class_signature_default_object_super_and_parameterized_super() {
        // No supers → plain generic class extends Object.
        let g = gsig(vec![("T", Ty::obj("kotlin/Any"))], vec![]);
        assert_eq!(
            jvm_class_signature(&g),
            Some("<T:Ljava/lang/Object;>Ljava/lang/Object;".to_string())
        );
        // A parameterized supertype is formatted from its Ty.
        let g2 = gsig(
            vec![("T", Ty::obj("kotlin/Any"))],
            vec![Ty::obj("kotlin/Any"), Ty::obj_args("demo/Box", &[Ty::Int])],
        );
        assert_eq!(
            jvm_class_signature(&g2),
            Some(
                "<T:Ljava/lang/Object;>Ljava/lang/Object;Ldemo/Box<Ljava/lang/Integer;>;"
                    .to_string()
            )
        );
    }
