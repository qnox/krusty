...
mod tests {
    ...
    #[test]
    fn desc_to_ty_covers_every_primitive_and_shape() {
        assert_eq!(desc_to_ty("I"), Ty::Int);
        assert_eq!(desc_to_ty("B"), Ty::Int); // Byte collapses to Int in a method descriptor
        assert_eq!(desc_to_ty("S"), Ty::Int); // Short collapses to Int
        assert_eq!(desc_to_ty("J"), Ty::Long);
        assert_eq!(desc_to_ty("F"), Ty::Float);
        assert_eq!(desc_to_ty("D"), Ty::Double);
        assert_eq!(desc_to_ty("Z"), Ty::Boolean);
        assert_eq!(desc_to_ty("C"), Ty::Char);
        assert_eq!(desc_to_ty("V"), Ty::Unit);
        assert_eq!(desc_to_ty(type_descriptor(Ty::String).as_str()), Ty::String);
        assert_eq!(desc_to_ty("[I"), Ty::array(Ty::Int));
        assert_eq!(desc_to_ty("[Ljava/lang/String;"), Ty::array(Ty::String));
        // A JVM built-in normalizes to its Kotlin identity.
        assert_eq!(desc_to_ty("Ljava/lang/Object;"), Ty::obj("kotlin/Any"));
        // A function interface descriptor becomes a semantic function type of the right arity.
        assert_eq!(
            desc_to_ty("Lkotlin/jvm/functions/Function2;"),
            Ty::fun(
                vec![Ty::obj("kotlin/Any"), Ty::obj("kotlin/Any")],
                Ty::obj("kotlin/Any")
            )
        );
        // A malformed / unknown descriptor is an error type.
        assert_eq!(desc_to_ty("Q"), Ty::Error);
        assert_eq!(desc_to_ty(""), Ty::Error);
    }

    #[test]
    fn field_desc_to_ty_keeps_byte_and_short_distinct() {
        assert_eq!(field_desc_to_ty("B"), Ty::Byte);
        assert_eq!(field_desc_to_ty("S"), Ty::Short);
        assert_eq!(field_desc_to_ty("[B"), Ty::array(Ty::Byte));
        // Everything else delegates to `desc_to_ty`.
        assert_eq!(field_desc_to_ty("I"), Ty::Int);
        assert_eq!(field_desc_to_ty("J"), Ty::Long);
    }

    #[test]
    fn function_iface_arity_parses_only_the_function_family() {
        assert_eq!(
            function_iface_arity("kotlin/jvm/functions/Function0"),
            Some(0)
        );
        assert_eq!(
            function_iface_arity("kotlin/jvm/functions/Function3"),
            Some(3)
        );
        // The bare `Function` (no digits) has no arity.
        assert_eq!(function_iface_arity("kotlin/jvm/functions/Function"), None);
        assert_eq!(function_iface_arity("java/lang/String"), None);
    }

    #[test]
    fn split_one_peels_a_single_descriptor() {
        assert_eq!(split_one("IJ"), Some(("I", "J")));
        assert_eq!(
            split_one("Ljava/lang/String;X"),
            Some(("Ljava/lang/String;", "X"))
        );
        assert_eq!(split_one("[[I"), Some(("[[I", "")));
        assert_eq!(split_one(""), None);
        // Trailing array marker with nothing after it is incomplete.
        assert_eq!(split_one("["), None);
    }

    #[test]
    fn descriptor_narrowing_counts_only_top_level_byte_short() {
        assert_eq!(descriptor_narrowing("(BS)V"), 2);
        assert_eq!(descriptor_narrowing("()V"), 0);
        // `B`/`S` inside a class name must NOT count.
        assert_eq!(descriptor_narrowing("(Ljava/lang/Boolean;)V"), 0);
        // An array marker is skipped; the following primitive still counts.
        assert_eq!(descriptor_narrowing("([BI)V"), 1);
        assert_eq!(descriptor_narrowing("(IJ)V"), 0);
    }

    #[test]
    fn kotlin_simple_name_of_ty_only_signed_numerics() {
        assert_eq!(kotlin_simple_name_of_ty(Ty::Int), Some("Int"));
        assert_eq!(kotlin_simple_name_of_ty(Ty::Long), Some("Long"));
        assert_eq!(kotlin_simple_name_of_ty(Ty::Double), Some("Double"));
        assert_eq!(kotlin_simple_name_of_ty(Ty::Float), Some("Float"));
        assert_eq!(kotlin_simple_name_of_ty(Ty::Byte), Some("Byte"));
        assert_eq!(kotlin_simple_name_of_ty(Ty::Short), Some("Short"));
        // Unsigned and non-numeric bail.
        assert_eq!(kotlin_simple_name_of_ty(Ty::UInt), None);
        assert_eq!(kotlin_simple_name_of_ty(Ty::Boolean), None);
        assert_eq!(kotlin_simple_name_of_ty(Ty::String), None);
    }

    #[test]
    fn source_internal_of_ty_maps_and_unwraps() {
        assert_eq!(source_internal_of_ty(Ty::Int), Some("kotlin/Int"));
        assert_eq!(source_internal_of_ty(Ty::Boolean), Some("kotlin/Boolean"));
        assert_eq!(source_internal_of_ty(Ty::UInt), Some("kotlin/UInt"));
        assert_eq!(source_internal_of_ty(Ty::ULong), Some("kotlin/ULong"));
        assert_eq!(source_internal_of_ty(Ty::String), Some("kotlin/String"));
        assert_eq!(source_internal_of_ty(Ty::obj("app/Foo")), Some("app/Foo"));
        // Nullable / type-parameter wrappers unwrap to the inner type's internal.
        assert_eq!(
            source_internal_of_ty(Ty::nullable(Ty::Int)),
            Some("kotlin/Int")
        );
        assert_eq!(
            source_internal_of_ty(Ty::ty_param("T", Ty::obj("app/Bound"))),
            Some("app/Bound")
        );
        // A type with no source internal (`Unit`) returns None.
        assert_eq!(source_internal_of_ty(Ty::Unit), None);
    }

    #[test]
    fn ty_simple_name_is_last_segment() {
        assert_eq!(ty_simple_name(Ty::Int), Some("Int"));
        assert_eq!(ty_simple_name(Ty::UInt), Some("UInt"));
        assert_eq!(ty_simple_name(Ty::obj("app/pkg/Foo")), Some("Foo"));
        assert_eq!(ty_simple_name(Ty::Unit), None);
    }

    #[test]
    fn strip_continuation_param_drops_trailing_cps_param() {
        assert_eq!(
            strip_continuation_param("(ILkotlin/coroutines/Continuation;)Ljava/lang/Object;"),
            "(I)Ljava/lang/Object;"
        );
        // No trailing continuation is a no-op.
        assert_eq!(strip_continuation_param("(I)V"), "(I)V");
        assert_eq!(
            strip_continuation_param("(Lkotlin/coroutines/Continuation;)Ljava/lang/Object;"),
            "()Ljava/lang/Object;"
        );
    }

    #[test]
    fn parse_method_desc_variants() {
        assert_eq!(
            parse_method_desc("(IJ)Z"),
            (vec![Ty::Int, Ty::Long], Ty::Boolean)
        );
        assert_eq!(parse_method_desc("()V"), (vec![], Ty::Unit));
        // The field-param variant keeps Byte/Short distinct.
        assert_eq!(
            parse_method_desc_with_field_params("(BS)V"),
            (vec![Ty::Byte, Ty::Short], Ty::Unit)
        );
    }

    #[test]
    fn member_matches_query_handles_aliases() {
        assert!(member_matches_query("foo", "foo"));
        assert!(member_matches_query("keySet", "keys"));
        assert!(member_matches_query("entrySet", "entries"));
        assert!(!member_matches_query("foo", "bar"));
        assert!(!member_matches_query("keys", "keySet"));
    }

    #[test]
    fn metadata_return_ty_canonicalizes() {
        assert_eq!(metadata_return_ty(Some("kotlin/Int")), Some(Ty::Int));
        assert_eq!(metadata_return_ty(Some("kotlin/String")), Some(Ty::String));
        assert_eq!(
            metadata_return_ty(Some("app/Foo")),
            Some(Ty::obj("app/Foo"))
        );
        assert_eq!(metadata_return_ty(None), None);
    }

    #[test]
    fn parse_formals_extracts_names() {
        assert_eq!(
            parse_formals("<T:Ljava/lang/Object;>rest"),
            (vec!["T".to_string()], "rest")
        );
        assert_eq!(
            parse_formals("<A:Ljava/lang/Object;B:Ljava/lang/Object;>X"),
            (vec!["A".to_string(), "B".to_string()], "X")
        );
        // No angle block: empty formals, unchanged input.
        assert_eq!(parse_formals("noangle"), (Vec::new(), "noangle"));
    }

    #[test]
    fn parse_gsig_covers_each_node() {
        // Type variable.
        assert!(matches!(parse_gsig("TT;"), Some((GSig::Var(n), "")) if n == "T"));
        // Primitives.
        assert!(matches!(parse_gsig("I"), Some((GSig::Prim(Ty::Int), ""))));
        assert!(matches!(parse_gsig("J"), Some((GSig::Prim(Ty::Long), ""))));
        assert!(matches!(parse_gsig("V"), Some((GSig::Prim(Ty::Unit), ""))));
        // Array of primitive.
        assert!(matches!(
            parse_gsig("[I"),
            Some((GSig::Arr(inner), "")) if matches!(*inner, GSig::Prim(Ty::Int))
        ));
        // Class (JVM built-in canonicalized to Kotlin).
        assert!(matches!(
            parse_gsig("Ljava/lang/String;"),
            Some((GSig::Class(n, args), "")) if n == "kotlin/String" && args.is_empty()
        ));
        // Function interface becomes a Function node (arity 1 = one param + ret).
        assert!(matches!(
            parse_gsig("Lkotlin/jvm/functions/Function1<TA;TB;>;"),
            Some((GSig::Function { params, ret }, ""))
                if params.len() == 1 && matches!(*ret, GSig::Var(ref r) if r == "B")
        ));
        // Wildcard argument becomes `kotlin/Any`.
        assert!(matches!(
            parse_gsig("Ljava/util/List<*>;"),
            Some((GSig::Class(_, args), "")) if args.len() == 1
        ));
        // Unknown leading byte fails.
        assert!(parse_gsig("Q").is_none());
        assert!(parse_gsig("").is_none());
    }

    #[test]
    fn gsig_unbox_wrapper_unboxes_known_wrappers() {
        assert!(matches!(
            gsig_unbox_wrapper(GSig::Class("java/lang/Integer".into(), vec![])),
            GSig::Prim(Ty::Int)
        ));
        assert!(matches!(
            gsig_unbox_wrapper(GSig::Class("kotlin/Int".into(), vec![])),
            GSig::Prim(Ty::Int)
        ));
        assert!(matches!(
            gsig_unbox_wrapper(GSig::Class("java/lang/Boolean".into(), vec![])),
            GSig::Prim(Ty::Boolean)
        ));
        // A non-wrapper class stays a class.
        assert!(matches!(
            gsig_unbox_wrapper(GSig::Class("app/Foo".into(), vec![])),
            GSig::Class(n, _) if n == "app/Foo"
        ));
        // A non-class node is returned untouched.
        assert!(matches!(
            gsig_unbox_wrapper(GSig::Var("T".into())),
            GSig::Var(n) if n == "T"
        ));
    }

    #[test]
    fn parse_method_gsig_and_class_gsig() {
        let m = parse_method_gsig("<T:Ljava/lang/Object;>(TT;)TT;").unwrap();
        assert_eq!(m.formals, vec!["T".to_string()]);
        assert_eq!(m.params.len(), 1);
        assert!(matches!(&m.params[0], GSig::Var(n) if n == "T"));
        assert!(matches!(&m.ret, GSig::Var(n) if n == "T"));

        let (formals, supers) =
            parse_class_gsig("<E:Ljava/lang/Object;>Ljava/lang/Object;Ljava/util/List<TE;>;")
                .unwrap();
        assert_eq!(formals, vec!["E".to_string()]);
        assert_eq!(supers.len(), 2);
        assert!(matches!(&supers[0], GSig::Class(n, _) if n == "kotlin/Any"));
        assert!(
            matches!(&supers[1], GSig::Class(n, args) if n == "java/util/List" && args.len() == 1)
        );
    }

    #[test]
    fn nonpublic_ext_receiver_is_typevar_checks_first_param() {
        assert!(nonpublic_ext_receiver_is_typevar(Some(
            "<T:Ljava/lang/Object;>(TT;)TT;"
        )));
        // A concrete-class receiver is not a type variable.
        assert!(!nonpublic_ext_receiver_is_typevar(Some(
            "(Ljava/lang/String;)V"
        )));
        assert!(!nonpublic_ext_receiver_is_typevar(None));
    }

    #[test]
    fn canonicalize_jvm_collections_rewrites_recursively() {
        assert_eq!(
            canonicalize_jvm_collections(Ty::obj_args("java/util/List", &[Ty::obj("app/Item")])),
            Ty::obj_args("kotlin/collections/List", &[Ty::obj("app/Item")])
        );
        // Nested inside an array, and a non-collection type passes through.
        assert_eq!(
            canonicalize_jvm_collections(Ty::array(Ty::obj_args("java/util/Set", &[Ty::Int]))),
            Ty::array(Ty::obj_args("kotlin/collections/Set", &[Ty::Int]))
        );
        assert_eq!(canonicalize_jvm_collections(Ty::Int), Ty::Int);
        assert_eq!(
            canonicalize_jvm_collections(Ty::obj("app/Plain")),
            Ty::obj("app/Plain")
        );
    }

    #[test]
    fn concrete_generic_ret_only_for_fully_concrete() {
        let concrete = GenericSig {
            formals: vec![],
            params: vec![],
            ret: GSig::Class(
                "kotlin/collections/List".into(),
                vec![GSig::Class("app/Item".into(), vec![])],
            ),
        };
        assert_eq!(
            concrete_generic_ret(&concrete),
            Some(Ty::obj_args(
                "kotlin/collections/List",
                &[Ty::obj("app/Item")]
            ))
        );
        // A return naming a type variable is not recovered here.
        let with_var = GenericSig {
            formals: vec![],
            params: vec![],
            ret: GSig::Class(
                "kotlin/collections/List".into(),
        ...
        assert_eq!(concrete_generic_ret(&with_var), None);
        // An argument-less class carries nothing to recover.
        let bare = GenericSig {
            formals: vec![],
            params: vec![],
            ret: GSig::Class("app/Foo".into(), vec![]),
        };
        assert_eq!(concrete_generic_ret(&bare), None);
    }

    #[test]
    fn gsig_receiver_element_unboxes_container_element() {
        assert_eq!(
            gsig_receiver_element(Some("(Ljava/lang/Iterable<Ljava/lang/Integer;>;)I")),
            Some(Ty::Int)
        );
        // No signature, and a non-container first param, yield nothing.
        assert_eq!(gsig_receiver_element(None), None);
        assert_eq!(gsig_receiver_element(Some("(I)V")), None);
    }

    #[test]
    fn suspend_return_from_gsig_recovers_continuation_arg() {
        let binds = std::collections::HashMap::new();
        // Bare class continuation argument → its canonical Ty.
        let g = GenericSig {
            formals: vec![],
            params: vec![GSig::Class(
                "kotlin/coroutines/Continuation".into(),
                vec![GSig::Class("kotlin/String".into(), vec![])],
            )],
            ret: GSig::Prim(Ty::Unit),
        };
        assert_eq!(suspend_return_from_gsig(&g, &binds), Some(Ty::String));

        // A generic (java) collection argument is canonicalized to its Kotlin type.
        let g2 = GenericSig {
            formals: vec![],
            params: vec![GSig::Class(
                "kotlin/coroutines/Continuation".into(),
        ...
        assert_eq!(
            suspend_return_from_gsig(&g2, &binds),
            Some(Ty::obj_args(
                "kotlin/collections/List",
                &[Ty::obj("app/Item")]
            ))
        ...
        // Last param is not a Continuation → not a suspend return.
        let not_suspend = GenericSig {
            formals: vec![],
            params: vec![GSig::Class("kotlin/String".into(), vec![])],
            ret: GSig::Prim(Ty::Unit),
        };
        assert_eq!(suspend_return_from_gsig(&not_suspend, &binds), None);
    }
}
