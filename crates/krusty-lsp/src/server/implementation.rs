...
pub trait Analysis {
    ...
    /// Analyze open documents with their URIs. Backends that model a project may use the paths to
    /// add unopened source files to the compiler source set, while still returning one result per
    /// open document.
    ...
    /// Record a watched-file change. Returns true when source analysis, rather than the project
    /// model, changed and open documents should be refreshed immediately.
    ...
    fn refresh_documents(&mut self) -> Vec<Value> {
        ...
        let analyses = if self.analyze.analysis_ready() {
            ...
            let open_documents = uris
                ...
                .collect::<Vec<_>>();
            let (analyses, support_documents) =
                self.analyze.analyze_open_documents(&open_documents);
