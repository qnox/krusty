...
fn companion_object_source(
    ...
    while index < class_close {
        index = skip_kotlin_trivia(bytes, index, class_close);
        ...
        match bytes[index] {
            ...
            _ if depth == 0 && word_at(bytes, index, b"companion") => {
                ...
                let object_lo = skip_kotlin_trivia(bytes, index + 9, class_close);
                if !word_at(bytes, object_lo, b"object") {
                    index += 9;
                    continue;
                }
                let object = Span::new(object_lo as u32, (object_lo + 6) as u32);
                let after_object = skip_kotlin_trivia(bytes, object_lo + 6, class_close);
...
fn find_declaration_body(bytes: &[u8], mut index: usize, end: usize) -> Option<usize> {
    ...
    while index < end {
        index = skip_kotlin_trivia(bytes, index, end);
...
fn skip_kotlin_trivia(bytes: &[u8], mut index: usize, end: usize) -> usize {
    loop {
...
fn skip_quoted(bytes: &[u8], start: usize, end: usize) -> usize {
    ...
    let triple = quote == b'"' && bytes.get(start..start + 3) == Some(b"\"\"\"");
...
fn source_prefix_is_deprecated(source: &str, prefix_lo: u32, declaration_lo: u32) -> bool {
    ...
    while index < end {
        match bytes[index] {
            ...
            b'"' | b'\'' | b'`' => index = skip_quoted(bytes, index, end),
            b'@' => {
                let mut cursor = skip_kotlin_trivia(bytes, index + 1, end);
                ...
                if bytes.get(cursor) == Some(&b':') {
                    cursor = skip_kotlin_trivia(bytes, cursor + 1, end);
                ...
                loop {
                    cursor = skip_kotlin_trivia(bytes, cursor, end);
                    if bytes.get(cursor) != Some(&b'.') {
                        break;
                    }
                    cursor = skip_kotlin_trivia(bytes, cursor + 1, end);
...
fn primary_constructor_spans(
    ...
    let after_name = skip_kotlin_trivia(bytes, index, end);
    ...
    if bytes.get(index) == Some(&b'<') {
        index = matching_delimiter(bytes, index, b'<', b'>')?.saturating_add(1);
    }
    let prefix_start = skip_kotlin_trivia(bytes, index, end);
    ...
    let close = matching_delimiter(bytes, open, b'(', b')')?;
...
fn skip_source_annotation(bytes: &[u8], at: usize, end: usize) -> usize {
    let mut index = skip_kotlin_trivia(bytes, at.saturating_add(1), end);
    ...
    loop {
        ...
        if bytes.get(index) == Some(&b':') || bytes.get(index) == Some(&b'.') {
            index = skip_kotlin_trivia(bytes, index + 1, end);
            if let Some(span) = source_identifier_span(bytes, index, end) {
                index = span.hi as usize;
                continue;
            }
    ...
    index = skip_kotlin_trivia(bytes, index, end);
    if bytes.get(index) == Some(&b'(') {
        matching_delimiter(bytes, index, b'(', b')').map_or(end, |close| close.saturating_add(1))
    } else {
        index
    }
}
...
fn enum_entry_range(source: &str, selection: Span, class_hi: u32, prefix_boundary: usize) -> Span {
    ...
    let lo = skip_kotlin_trivia(bytes, prefix_boundary, selection.lo as usize);
    while hi > lo && bytes.get(hi - 1).is_some_and(u8::is_ascii_whitespace) {
        hi -= 1;
    }
    Span::new(lo as u32, hi as u32)
