...
//! is LARGE pushes a relocated `ldc` (1-byte pool index) past 255 — an emergent whole-file failure
//! because the require/check inline splice compiled in isolation but not in the full fixture.
//! `relocate_insns` now widens such an `ldc` to `ldc_w` (0x13, the
