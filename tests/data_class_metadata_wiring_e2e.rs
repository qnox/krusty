...
/// directly from the property declaration. The record therefore has to take the value-class pass's
/// stamp — the declared type `K` and the exact mangled accessor — plus the explicit erased field
/// descriptor a reader cannot derive from `K`. Describing plain `getK` would advertise a method the
/// class file does not define.
#[test]
fn value_class_body_property_is_byte_identical() {
    assert_byte_identical(
        "package demo\n\n@JvmInline\nvalue class K(val v: String)\n\nclass Holder {\n    val k: K = K(\"OK\")\n}\n",
        "demo/Holder",
        &[],
...
/// An `is`-prefixed property keeps the SOURCE name as its accessor (`val isOpen` → `isOpen()`, never
/// `getIsOpen`; a `var`'s setter is `setOpen`). The record, the constant-pool seeder and the debug
/// tables all key on the accessor by name, so each had to ask the same naming helper the emission does:
/// while they spelled `getIsOpen`, the record advertised a method the class file does not define, the
/// pool carried a constant nothing referenced, and the real accessor lost its debug tables.
#[test]
fn is_prefixed_property_accessors_are_byte_identical() {
    assert_byte_identical(
