...
return \"OK\"\n\
    ...
    let kc = Command::new(krusty).args(["-d", dir.to_str().unwrap()]).arg(dir.join("L.kt")).output().unwrap();
    if !kc.status.success() { eprintln!("skip (IR unsupported): {}", String::from_utf8_lossy(&kc.stderr)); return; }
    fs::write(dir.join("M.java"), "public class M { public static void main(String[] a) { System.out.println(LKt.box()); } }").unwrap();
