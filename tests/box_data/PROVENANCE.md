...
is run directly against a Kotlin checkout by `tests/kotlin_box_conformance.rs` (gated on
`KRUSTY_KOTLIN_BOX_DIR`), which skips everything krusty can't yet compile and asserts krusty never
miscompiles a case it accepts.

To refresh / widen this set, point `KRUSTY_KOTLIN_BOX_DIR` at a Kotlin checkout and copy over any
newly-passing cases.
