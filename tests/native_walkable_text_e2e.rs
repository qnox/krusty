...
/// is visible: one `get` per character, and `length` asked once before each step and once to end
/// the walk — never again inside a step, where the program's getter would see a second call.
#[test]
fn a_programs_own_char_sequence_is_walked_with_index() {
    let source = "class Counting(private val s: String) : CharSequence {\n\
         ...
         \x20   if (cs.lengthCalls != 4) return \"fail length \" + cs.lengthCalls\n\
         \x20   return \"OK\"\n\
         }\n";
