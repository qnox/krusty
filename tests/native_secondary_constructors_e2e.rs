...
fn a_supertype_list_reaches_the_constructor_the_checker_selected() {
    ...
    // Native only: the JVM backend emits a sealed class's secondary constructor as `private`, so
    // `D` cannot reach it there (`IllegalAccessError`), which is that backend's defect to fix.
