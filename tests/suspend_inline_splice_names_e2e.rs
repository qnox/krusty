...
///
/// Deliberately NOT covered here, because this change does not fix them: an inline lambda with an
/// extension RECEIVER additionally spills that receiver as `$this$<enclosing>$lambda$<n>`, which
/// needs kotlinc's synthetic lambda numbering (krusty names the same function `<enclosing>$fir_…`);
/// and the `L$N` slot each spill lands in still differs for this shape. Both are separate changes.
#[test]
fn a_spliced_inline_body_names_its_locals_with_iv() {
    ...
    let source = "package demo\n\
        ...
        inline fun buildSpec(urlString: String, block: () -> String): String {\n\
        \x20 val builder = StringBuilder()\n\
        \x20 builder.append(block())\n\
        \x20 return urlString + builder.toString()\n\
        }\n\
        class Caller(private val client: Client) {\n\
        \x20 suspend fun nested(tag: String): String =\n\
        \x20\x20 client.send(buildSpec(\"root\") { client.send(tag) })\n\
        }\n";
    ...
    assert_eq!(
        reference_names,
        ["tag", "urlString$iv", "builder$iv", "tag"],
        "the fixture must exercise an inline expansion's own locals"
    );
    assert_eq!(
        ours_names, reference_names,
        "continuation spill names must match kotlinc exactly"
    );
}
