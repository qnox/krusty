...
fn debug_metadata(bytes: &[u8], file_name: &str) -> Vec<String> {
    ...
    assert_eq!(
        rows.len(),
        2,
        "expected one `s` row and one `n` row in:\n{text}"
    );
    rows
}
...
fn assert_spill_names(name: &str, source: &str, class_name: &str, expected: &[&str]) {
    ...
    let reference_rows = debug_metadata(&reference, &format!("ref-{simple}.class"));
    assert_eq!(
        reference_rows, expected,
        "{name}: the fixture must exercise the inline expansion kotlinc names"
    );
    assert_eq!(
...
fn a_spliced_map_spills_its_inline_locals() {
    assert_spill_names(
        ...
        "demo/Fan$spread$1",
        &[
            "s=[\"L$0\",\"L$1\",\"L$0\",\"L$1\",\"L$2\",\"L$3\",\"L$4\",\"L$5\",\"L$7\",\"L$8\"]",
            "n=[\"ids\",\"tag\",\"ids\",\"tag\",\"session\",\"$this$map$iv\",\"$this$mapTo$iv$iv\",\"destination$iv$iv\",\"item$iv$iv\",\"one\"]",
        ],
    );
}
...
fn a_spliced_flat_map_spills_its_inline_locals() {
    assert_spill_names(
        ...
        &[
            "s=[\"L$0\",\"L$1\",\"L$0\",\"L$1\",\"L$2\",\"L$3\",\"L$4\",\"L$5\",\"L$7\",\"L$8\"]",
            "n=[\"ids\",\"tag\",\"ids\",\"tag\",\"session\",\"$this$flatMap$iv\",\"$this$flatMapTo$iv$iv\",\"destination$iv$iv\",\"element$iv$iv\",\"one\"]",
        ],
    );
}
