...
/// [`compile_in_process_files`] with kotlinc's `-java-parameters`: the backend writes a
/// `MethodParameters` attribute naming each declared parameter.
#[allow(dead_code)]
pub fn compile_in_process_files_java_parameters(
    sources: &[(&str, &str)],
    cp_jars: &[PathBuf],
    jdk_modules: Option<&std::path::Path>,
) -> Option<Vec<(String, Vec<u8>)>> {
    ...
    let outputs = krusty::compiler::emit_analyzed(
        analysis,
        &stems,
        &krusty::jvm::JvmBackend::new(cp).with_java_parameters(true),
        "main",
        &mut diags,
    );
    let classes = outputs
