...
/// [`run_box_against`] with `kotlin-reflect` on the compile/runtime classpath, for assertions that
/// exercise a callable reference's reflection owner/signature rather than only its generated invoke.
#[allow(dead_code)]
