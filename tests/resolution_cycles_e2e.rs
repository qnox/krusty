...
fn assert_declines(tag: &str, names: &[&str], sources: &[&str]) {
    if !common::stdlib_toolchain_ready() {
        return;
    }
    let reported = diagnostics(sources);
    for name in names {
        let wanted = format!("cannot infer the type of property '{name}'");
        assert!(
            reported.iter().any(|line| line.contains(&wanted)),
            "{tag}: expected a decline for '{name}', got {reported:?}"
        );
    }
}
...
fn a_property_initialized_from_itself_declines() {
    assert_declines("self", &["a"], &["val a = a\n"]);
}
...
fn two_properties_initialized_from_each_other_decline() {
    ...
    assert_declines("mutual", &["a", "b"], &["val a = b\nval b = a\n"]);
}
...
fn a_cycle_spanning_files_declines_in_either_order() {
    ...
    assert_declines("cross-file", &["a", "b"], &["val a = b\n", "val b = a\n"]);
    assert_declines(
        "cross-file-reversed",
        &["a", "b"],
        &["val b = a\n", "val a = b\n"],
