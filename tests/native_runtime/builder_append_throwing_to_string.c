...
void kt_program_entry(void) {
    ...
    (void)kt_string_builder_append(builder, throwing);
    CHECK(kt_pending_exception() != NULL, "the toString exception did not propagate\n");
    kt_clear_pending();
    CHECK(text_is(builder, "kept", 4), "a failed append changed the builder\n");

    (void)kt_string_builder_append(builder, kt_string_utf8("!", 1));
    CHECK(text_is(builder, "kept!", 5), "the builder stopped taking appends\n");
    kt_sys_write(1, "OK\n", 3);
}
