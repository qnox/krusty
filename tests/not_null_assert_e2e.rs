...
fn not_null_assert_runs() {
    ...
    let dir = std::env::temp_dir().join(format!("krusty_nn_{}", std::process::id()));
...
return \"OK\"\n\
    ...
    fs::write(dir.join("N.kt"), src).unwrap();
    let kc = Command::new(krusty).args(["-d", dir.to_str().unwrap()]).arg(dir.join("N.kt")).output().unwrap();
    if !kc.status.success() { eprintln!("skip (IR unsupported): {}", String::from_utf8_lossy(&kc.stderr)); return; }
    fs::write(dir.join("M.java"), "public class M { public static void main(String[] a) { System.out.println(NKt.box()); } }").unwrap();
