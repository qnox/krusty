//! Autoboxing: a primitive flowing to `Any`/`Object` (or an erased generic) boxes to its wrapper;
//! the reverse unboxes. The primitive-vs-boxed representation is an emit concern, not the checker's.
...
const SRC: &str = r#"
fun box(): String {
    val a: Any = 5            // box Int -> Integer
    val n: Int = a            // unbox Integer -> int
    val b: Any = 2.5          // box Double
    val d: Double = b         // unbox
    val c: Any = 'x'          // box Char
    val ch: Char = c          // unbox
    return if (n == 5 && d == 2.5 && ch == 'x') "OK" else "no n=$n d=$d ch=$ch"
}
"#;

#[test]
fn boxing_roundtrip_runs() {
    let Ok(java_home) = std::env::var("KRUSTY_REF_JAVA_HOME").or_else(|_| std::env::var("JAVA_HOME")) else {
        eprintln!("skipping boxing_e2e: set JAVA_HOME");
        return;
    };
    ...
    let dir = std::env::temp_dir().join(format!("krusty_boxing_{}", std::process::id()));
    let _ = fs::remove_dir_all(&dir);
    fs::create_dir_all(&dir).unwrap();
    let internal = file_class_name("Boxing", None);
