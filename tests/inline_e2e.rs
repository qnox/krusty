...
fn splices_real_empty_array_body() {
    ...
    assert!(
        pre.iter()
            .any(|i| matches!(i, Insn::Plain { op: 0xb8, .. })),
        "raw body invokes the marker"
    );
    // Recognition gate: emptyArray is a reified-inline function (must be inlined, not called).
    assert!(
        is_reified_inline(&body),
        "emptyArray recognized as reified-inline by its body"
    );
    ...
    let spliced =
        splice(&body, "()[Ljava/lang/Object;", 5, &tm, &mut cw).expect("splice real body");
    ...
    assert!(
        spliced
            .iter()
            .any(|i| matches!(i, Insn::Plain { op: 0xbd, .. })),
        "spliced body creates an array (anewarray)"
    );
    assert!(
        !spliced
            .iter()
            .any(|i| matches!(i, Insn::Plain { op: 0xb8, .. })),
        "the reified marker call is gone"
    );
    assert!(
        spliced
            .iter()
            .any(|i| matches!(i, Insn::Branch { op: 0xa7, .. })),
        "return redirected to goto"
    );
    ...
    let anew = spliced
        ...
        .find_map(|i| match i {
            Insn::Plain { op: 0xbd, operands } => {
                Some((operands[0] as u16) << 8 | operands[1] as u16)
            }
            _ => None,
        })
    ...
    assert_eq!(
        anew, expected,
        "anewarray uses the reified String, not Object"
    );
