//! Context function type checking and lowering.
use super::common;
...
#[test]
fn typed_context_lambda_initializer_uses_implicit_receiver() {
    const SRC: &str = "// LANGUAGE: +ContextParameters\n\
class Session(val token: String)\n\
val action: context(Session) () -> String = { token }\n\
fun box(): String = action(Session(\"OK\"))\n";
    common::expect_box_ok_with_stdlib(SRC, "TypedContextLambda");
}

#[test]
fn typed_context_extension_lambda_initializer_uses_implicit_receivers() {
    const SRC: &str = "// LANGUAGE: +ContextParameters\n\
class Session(val prefix: String)\n\
class Target(val suffix: String)\n\
val action: context(Session) Target.() -> String = { prefix + suffix }\n\
fun box(): String = action(Session(\"O\"), Target(\"K\"))\n";
    common::expect_box_ok_with_stdlib(SRC, "TypedContextExtensionLambda");
}

#[test]
fn context_extension_lambda_return_uses_implicit_receivers() {
    const SRC: &str = "// LANGUAGE: +ContextParameters\n\
class Session(val prefix: String)\n\
class Target(val suffix: String)\n\
fun action(): context(Session) Target.() -> String = { prefix + suffix }\n\
fun box(): String = action()(Session(\"O\"), Target(\"K\"))\n";
    common::expect_box_ok_with_stdlib(SRC, "ContextExtensionLambdaReturn");
}

#[test]
fn constructor_context_lambda_uses_implicit_receiver() {
    const SRC: &str = "// LANGUAGE: +ContextParameters\n\
class Session(val token: String)\n\
class Runner(val action: context(Session) () -> String)\n\
fun box(): String = Runner { token }.action(Session(\"OK\"))\n";
    common::expect_box_ok_with_stdlib(SRC, "ConstructorContextLambda");
}

#[test]
fn local_function_context_lambda_uses_implicit_receiver() {
    const SRC: &str = "// LANGUAGE: +ContextParameters\n\
class Session(val token: String)\n\
fun box(): String {\n\
    fun inspect(session: Session, action: context(Session) () -> String): String = action(session)\n\
    return inspect(Session(\"OK\")) { token }\n\
}\n";
    common::expect_box_ok_with_stdlib(SRC, "LocalContextLambda");
}
