...
//! value_class_unboxed_arith). These tests drive the production streaming compiler and inspect the
//! synthesized class shape — the structural half of the differential-vs-kotlinc check.
...
fn value_class_synthesizes_box_unbox_constructor_impl() {
    ...
    let classes =
        common::compile_in_process(src, "S", &[common::stdlib_jar()], Some(jdk.as_path()))
...
fn value_class_is_property_uses_javabean_getter_name() {
    ...
    let jdk = common::jdk_modules();
    let classes =
        common::compile_in_process(src, "Flag", &[common::stdlib_jar()], Some(jdk.as_path()))
            .expect("production compiler should emit the value class");
