...
/// Missing tooling or a rejected source panics with diagnostics; neither can become a passing skip.
...
fun box(): String {\n\
    ...
    let out = run(SRC, "ImportObjMember");
    assert_eq!(out, "OK");
}
...
fun box(): String {\n\
    ...
    let out = run(SRC, "ThisLabelField");
    assert_eq!(out, "OK");
}
...
fun box(): String {\n\
    ...
    let out = run(SRC, "CompanionInstance");
    assert_eq!(out, "OK");
}
