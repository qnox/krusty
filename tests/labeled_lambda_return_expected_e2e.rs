...
fn errors(output: &str) -> Vec<ObservedError> {
    output
        ...
        .filter_map(|rendered| {
            ...
            let column = fields
                .next()
                .expect("error diagnostic must contain a column")
                .trim()
                .parse()
                .expect("error diagnostic column must be numeric");
            let line = fields
                .next()
                .expect("error diagnostic must contain a line")
                .trim()
                .parse()
                .expect("error diagnostic line must be numeric");
            let path = fields
                .next()
                .expect("error diagnostic must contain a file")
                .trim();
            Some(ObservedError {
                file: Path::new(path)
                    .file_name()
...
\x20   }\n";
    ...
    let expected = vec![ObservedError {
        ...
        line: 3,
        column: 31,
        message: "return type mismatch: expected 'List<Long>', actual 'String'.".to_string(),
    }];
    ...
    assert_eq!(reference, expected, "complete ordered kotlinc diagnostics");
