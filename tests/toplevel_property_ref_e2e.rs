...
fn toplevel_property_refs_run() {
    let Some(java_home) = common::java_home() else {
        eprintln!("skipping toplevel_property_ref_e2e: set JAVA_HOME");
        return;
    };
    let Some(stdlib) = common::stdlib_jar() else {
        eprintln!("skipping toplevel_property_ref_e2e: no kotlin-stdlib jar found");
        return;
    };
...
fun box(): String {\n\
    ...
    let Some(out) = common::compile_and_run_box(SRC, "PropRefKt", &[stdlib], Some(&jdk)) else {
        return;
    };
    assert_eq!(out, "OK");
}
