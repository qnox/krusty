...
# memory-heavy Kotlin box corpus tests isolated — BOTH of them, the JVM lane and the native one,
# each partitioned across shards by its own runner so no process exceeds the two-minute gate
# deadline — then run every other conformance test in a fresh process.
...
conformance-all-plain:
    just conformance
    just conformance-native-run "$(just conformance-bin)" "$(just max-version)"
...
conformance-one VERSION:
    ...
    just conformance-native-run "$bin" "$v"
